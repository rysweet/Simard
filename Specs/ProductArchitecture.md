...
- the engineer loop checkpoints its session at each phase boundary (Intake, Preparation, Planning, Execution) and, on startup, RESUMES a matching in-progress session from its last checkpoint instead of re-running the goal from scratch; resume is idempotent — a phase whose result was already recorded (including a completed agent Execution) is reused, never repeated, so an interruption (crash, restart, or deploy binary-swap) can never cause a double-PR or duplicate work; a checkpoint whose objective does not match the current goal is ignored, and this session-level safety complements the deploy-drain goal-requeue that keeps the GOAL safe
...
Stewardship Mode is the mechanism that makes that role concrete: Simard converts her own orchestrator failures into tracked, deduplicated GitHub issues without feeding automation output back into her goal board.
...
- **Issue contract.** New issues use a fixed trusted title and embed structured front-matter so future runs can match them: `filed-by: simard-stewardship`, `stewardship-signature: <hex>`, `originating-run`, `failed-step`, `source-module`, followed by the error. All issue-bound metadata and error text pass through final credential redaction; only the raw failure kind and error text feed the deduplication signature.
- **No recursive handoff.** Whether the cycle ended in `FiledNew` or `MatchedExisting`, the goal board remains unchanged. A stewardship issue is an external tracker, not new internal work.
