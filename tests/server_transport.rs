...
use simard::server_circuit::{CircuitBreakerConfig, CircuitBreakerTransport, CircuitState};
use simard::server_subprocess::SubprocessServerTransport;
use simard::server_transport::{
    ServerHealth, ServerRequest, ServerTransport, new_request_id, unpack_server_response,
};

fn echo_server_script() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/echo_server.py")
}

fn echo_transport() -> SubprocessServerTransport {
    SubprocessServerTransport::new(
        "echo-test",
        echo_server_script(),
        vec![],
...
fn health_request() -> ServerRequest {
    ServerRequest {
        id: new_request_id(),
...
#[test]
fn subprocess_adapter_health_check_roundtrips() {
...
#[test]
fn subprocess_adapter_echo_roundtrips_params() {
    let transport = echo_transport();
...
#[test]
fn subprocess_adapter_unknown_method_returns_error() {
    let transport = echo_transport();
    let request = ServerRequest {
        id: new_request_id(),
...
#[test]
fn subprocess_adapter_reuses_child_across_calls() {
    let transport = echo_transport();
    for i in 0..5 {
        let request = ServerRequest {
            id: new_request_id(),
...
#[test]
fn subprocess_adapter_unpack_typed_health_response() {
    let transport = echo_transport();
    let request = ServerRequest {
        id: new_request_id(),
        method: "bridge.health".to_string(),
    ...
    let health: ServerHealth =
        unpack_server_response("echo", "bridge.health", response).expect("unpack should succeed");
    assert_eq!(health.server_name, "echo");
}

// --- Feral: adapter script does not exist ---

#[test]
fn subprocess_adapter_missing_script_fails_with_adapter_error() {
    let transport = SubprocessServerTransport::new(
        "missing",
        "/nonexistent/path/adapter.py",
        vec![],
    ...
    // ServerSpawnFailed (if python3 itself isn't found) or
    // ServerTransportError (python3 exits after failing to open the file).
    match error {
        SimardError::ServerSpawnFailed { adapter, .. } => {
            assert_eq!(adapter, "missing");
        }
        SimardError::ServerTransportError { adapter, reason } => {
            assert_eq!(adapter, "missing");
            assert!(
        ...
        other => panic!("expected ServerSpawnFailed or ServerTransportError, got: {other}"),
    }
}

// --- Feral: adapter script that exits immediately ---

#[test]
fn subprocess_adapter_immediate_exit_returns_transport_error() {
    let script =
    ...
    let transport =
        SubprocessServerTransport::new("exit-test", &script, vec![], Duration::from_secs(2));
    let result = transport.call(health_request());
    assert!(result.is_err());
    match result.unwrap_err() {
        SimardError::ServerTransportError { reason, .. } => {
            assert!(
                reason.contains("closed stdout") || reason.contains("process exited"),
                "reason: {reason}"
            );
        }
        other => panic!("expected ServerTransportError, got: {other}"),
    }

    let _ = std::fs::remove_file(&script);
}

// --- Feral: adapter script that returns malformed JSON ---

#[test]
fn subprocess_adapter_malformed_json_returns_protocol_error() {
...
"#,
    ...
    let transport =
        SubprocessServerTransport::new("malformed", &script, vec![], Duration::from_secs(2));
    let result = transport.call(health_request());
    assert!(result.is_err());
    match result.unwrap_err() {
        SimardError::ServerProtocolError { reason, .. } => {
            assert!(reason.contains("malformed"), "reason: {reason}");
        }
        other => panic!("expected ServerProtocolError, got: {other}"),
    }
...
#[test]
fn circuit_breaker_passes_through_on_healthy_adapter() {
...
fn circuit_breaker_opens_on_repeated_transport_errors() {
    ...
    let inner = SubprocessServerTransport::new("failing", &script, vec![], Duration::from_secs(1));
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        SimardError::ServerCircuitOpen { .. } => {}
        other => panic!("expected ServerCircuitOpen, got: {other}"),
    }

    let _ = std::fs::remove_file(&script);
}

// --- Descriptor includes adapter name ---

#[test]
fn subprocess_adapter_descriptor_contains_adapter_name() {
    let transport = echo_transport();
