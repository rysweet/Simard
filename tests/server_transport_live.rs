...
fn live_memory_adapter_stores_and_retrieves_fact() {
    let state_root = test_state_root();
    let adapter = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_adapter_working_memory_lifecycle() {
    let state_root = test_state_root();
    let adapter = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
    ...
    // Get working memory
    let slots = adapter.get_working("session-001").expect("get_working");
    assert_eq!(slots.len(), 1);
    assert_eq!(slots[0].slot_type, "goal");
    assert!(slots[0].content.contains("auth bug"));

    // Clear working memory
    let cleared = adapter.clear_working("session-001").expect("clear_working");
    assert!(cleared >= 1);
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_adapter_episode_and_consolidation() {
    let state_root = test_state_root();
    let adapter = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
    ...
    let stats = adapter.get_statistics().expect("stats");
    assert!(stats.episodic_count >= 3);

    // Consolidation with batch_size > available episodes returns None
    let consolidated = adapter.consolidate_episodes(10).expect("consolidate");
    assert!(
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_adapter_procedure_store_and_recall() {
    let state_root = test_state_root();
    let adapter = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
