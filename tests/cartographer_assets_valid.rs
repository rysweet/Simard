...
#[test]
fn shipped_cartographer_system_prompt_describes_the_deliverables() {
    let prompt = std::fs::read_to_string(repo_path(
        "prompt_assets/simard/identities/cartographer/cartographer_system.md",
    ))
    .expect("cartographer system prompt should be readable");
    for needle in ["dashboard.html", "narrative.md", "analysis.json", "serve"] {
        assert!(
