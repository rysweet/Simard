//! Outside-in integration tests for CognitiveMemoryStoreAdapter.
//!
...
impl TestFixture {
    fn new(_label: &str) -> Self {
        ...
        let store = CognitiveMemoryStoreAdapter::new(native_mem, &local_store).expect("adapter");
        Self {
            store,
            state_root,
            local_store,
...
fn live_adapter_descriptor_identifies_backend() {
    ...
    assert!(
        desc.identity.contains("cognitive-adapter"),
        "descriptor should identify cognitive adapter backend, got: {}",
        desc.identity
    );
}
