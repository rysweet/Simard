//! Outside-in integration tests for CognitiveClientMemoryStore.
//!
...
use simard::memory_store_adapter::CognitiveClientMemoryStore;
use simard::session::SessionPhase;

/// RAII guard that cleans up test artifacts on drop.
struct TestFixture {
    store: CognitiveClientMemoryStore,
    state_root: PathBuf,
    local_store: PathBuf,
}
...
impl TestFixture {
    fn new(_label: &str) -> Self {
        ...
        let store = CognitiveClientMemoryStore::new(native_mem, &local_store).expect("adapter");
        Self {
            store,
            state_root,
            local_store,
