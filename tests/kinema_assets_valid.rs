//! Asset-level validation for the EXAMPLE `kinema` animation & motion-graphics
//! identity package.
//!
//! `kinema` is a DATA-ONLY example identity under
//! `examples/identities/kinema/` (identity.toml + prompts/ + recipes/). It is
...
/// The shipped `examples/identities/kinema/` package must load end-to-end
/// through the data-driven [`load_example_identity`] rail, in curator mode,
/// exposing all five phase prompts.
#[test]
fn shipped_kinema_example_identity_loads() {
    let manifest = load_example_identity(&example_base(), "kinema", &test_request("kinema"))
        .expect("the examples/identities/kinema package must load via load_example_identity");

    assert_eq!(manifest.name, "kinema");
    assert_eq!(manifest.default_mode, OperatingMode::Curator);
    assert_eq!(
        manifest.prompt_assets.len(),
        5,
        "kinema ships 5 phase prompts (system + storyboard + rig + render + deliver)"
    );
    ...
    for expected in [
        ...
        assert!(
            ...
            "kinema identity must expose the `{expected}` prompt asset"
        );
    }
}

/// The example identity declares a read-only write posture (it renders
/// artifacts into an operator-provided output dir; it does not need project
/// write authority).
#[test]
fn shipped_kinema_identity_is_read_only() {
    let manifest = load_example_identity(&example_base(), "kinema", &test_request("kinema"))
        .expect("kinema package must load");
    assert_eq!(
        manifest.authority.posture,
        WritePosture::ReadOnly,
        "kinema declares `posture = \"read-only\"` in identity.toml"
    );
    assert!(
        !manifest.memory_policy.allow_project_writes,
        "kinema memory policy must not allow project writes"
    );
}
...
#[test]
fn kinema_is_not_a_builtin_identity() {
    let builtin_err = BuiltinIdentityLoader
        .load(&test_request("kinema"))
        .expect_err("kinema must NOT be loadable via the BuiltinIdentityLoader — it is data-only");
...
#[test]
fn shipped_kinema_prompt_files_exist_and_are_staged() {
    let prompts = example_base().join("kinema/prompts");
    for (file, heading) in [
        ("kinema_system.md", "Kinema System Prompt"),
        ("kinema_storyboard.md", "Stage 1: Storyboarding"),
        ("kinema_rig.md", "Stage 2: Rigging & blocking"),
        ("kinema_render.md", "Stage 3: Rendering"),
        ("kinema_deliver.md", "Stage 4: Motion brief"),
    ] {
...
/// The goal-session recipe must exist and drive the four-stage animation
/// pipeline end-to-end, carrying the context vars the stages consume and the
/// external domain tooling the pipeline shells out to (Blender / Synfig /
/// Natron) — all of which lives in the recipe, never in `src/`.
#[test]
fn shipped_kinema_recipe_drives_the_animation_pipeline() {
    let recipe_path = example_base().join("kinema/recipes/kinema-animation.yaml");
    let recipe = std::fs::read_to_string(&recipe_path)
        .unwrap_or_else(|e| panic!("recipe {} must be readable: {e}", recipe_path.display()));

    for required in ["name:", "description:", "steps:", "kinema-animation"] {
        assert!(
    ...
    for tool in ["blender", "synfig", "Natron"] {
        assert!(
...
#[test]
fn missing_kinema_variant_fails_visibly() {
    let err = load_example_identity(&example_base(), "kinemaa", &test_request("kinemaa"))
