...
use simard::rpc::{RpcHealth, RpcRequest, RpcTransport, new_request_id, unpack_rpc_response};
use simard::rpc_circuit_breaker::{CircuitBreakerConfig, CircuitBreakerTransport, CircuitState};
use simard::rpc_transport::SubprocessRpcTransport;

fn echo_rpc_script() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/echo_rpc.py")
}

fn echo_transport() -> SubprocessRpcTransport {
    SubprocessRpcTransport::new(
        "echo-test",
        echo_rpc_script(),
        vec![],
...
fn subprocess_bridge_unpack_typed_health_response() {
    ...
    let health: RpcHealth =
        unpack_rpc_response("echo", "bridge.health", response).expect("unpack should succeed");
    assert_eq!(health.server_name, "echo");
}
...
fn subprocess_bridge_missing_script_fails_with_bridge_error() {
    ...
    match error {
        ...
        other => panic!("expected RpcSpawnFailed or RpcTransportError, got: {other}"),
    }
}
...
fn subprocess_bridge_immediate_exit_returns_transport_error() {
    ...
    let transport =
        SubprocessRpcTransport::new("exit-test", &script, vec![], Duration::from_secs(2));
    let result = transport.call(health_request());
    assert!(result.is_err());
    match result.unwrap_err() {
        SimardError::RpcTransportError { reason, .. } => {
            assert!(
                reason.contains("closed stdout") || reason.contains("process exited"),
                "reason: {reason}"
            );
        }
        other => panic!("expected RpcTransportError, got: {other}"),
    }
...
"#,
    ...
    let transport =
        SubprocessRpcTransport::new("malformed", &script, vec![], Duration::from_secs(2));
    let result = transport.call(health_request());
    assert!(result.is_err());
    match result.unwrap_err() {
        SimardError::RpcProtocolError { reason, .. } => {
            assert!(reason.contains("malformed"), "reason: {reason}");
        }
        other => panic!("expected RpcProtocolError, got: {other}"),
    }
...
fn circuit_breaker_opens_on_repeated_transport_errors() {
    ...
    let inner = SubprocessRpcTransport::new("failing", &script, vec![], Duration::from_secs(1));
