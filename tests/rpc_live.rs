...
fn live_memory_client_stores_and_retrieves_fact() {
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_client_working_memory_lifecycle() {
    let state_root = test_state_root();
    let bridge = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_client_episode_and_consolidation() {
    let state_root = test_state_root();
    let bridge = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
...
#[test]
#[ignore] // Requires LadybugDB — run with: cargo test -- --ignored
fn live_memory_client_procedure_store_and_recall() {
    let state_root = test_state_root();
    let bridge = LibraryCognitiveMemory::open(&state_root).expect("native memory should open");
