//! Asset-level validation for the shipped Atelier identity.
//!
//! These tests prove the Atelier *capability-bearing* assets are not just
...
//!   drives the `simard atelier` surface.
...
fn test_contract() -> ManifestContract {
    ManifestContract::new(
        "atelier::assets::test",
        "brief -> package",
        vec!["test:atelier".to_string()],
        Provenance::new("atelier-assets-test", "tests/atelier_assets_valid.rs"),
...
#[test]
fn shipped_atelier_identity_card_loads_and_is_engineer_mode() {
    let prompt_root = repo_path("prompt_assets");
    let identity_dir = prompt_root.join("simard/identities/atelier");
    assert!(
        identity_dir.join("identity.toml").exists(),
        "atelier identity card should be shipped at {}",
        identity_dir.display()
    );

    let loader = FileIdentityLoader::new(&identity_dir, &prompt_root);
    let request = IdentityLoadRequest::new("simard-atelier", "0.1.0", test_contract());
    let manifest = loader
        .load(&request)
        .expect("shipped atelier identity.toml should load");
    ...
    assert!(
        manifest
            .prompt_assets
            .iter()
            .any(|a| a.id.as_str() == "atelier-system"),
        "atelier identity should expose its system prompt asset"
    );
}

#[test]
fn shipped_atelier_capability_policy_parses_and_is_least_privilege() {
    let policy_path =
        repo_path("prompt_assets/simard/policies/atelier-goal-session-capabilities.toml");
    let policy = CapabilityPolicy::from_toml_file(&policy_path)
        .expect("atelier goal-session capability policy should parse and validate");
...
#[test]
fn shipped_atelier_recipes_drive_the_cli_command_surface() {
    for (relative, expected_command) in [
        (
            "prompt_assets/simard/recipes/atelier-parametric-modeling.yaml",
            "atelier build",
        ),
        (
