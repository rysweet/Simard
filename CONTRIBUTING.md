...
| `full_session_lifecycle_triggers_all_consolidation_phases` (`tests/memory_consolidation_lifecycle.rs`) | Pre-existing — `BridgeCallFailed { method: "memory.get_working", reason: "unknown: memory.get_working" }`. Python bridge is missing the `memory.get_working` handler. Re-verified after Workstream B (checkpoint API + WAL backups + SIGTERM shutdown): unchanged failure on the bridge side. | [#1640](https://github.com/rysweet/Simard/issues/1640) |
