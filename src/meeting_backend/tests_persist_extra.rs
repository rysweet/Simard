...
fn write_auto_save_lands_under_simard_state_root() {
    ...
    // SAFETY: serialized via `#[serial(simard_meetings_dir_env, cognitive_memory)]` —
    // `simard_meetings_dir_env` excludes EVERY test that reads or mutates the
    // process-global meetings-dir env vars (SIMARD_MEETINGS_DIR / SIMARD_MEETINGS_ROOT
    // / SIMARD_STATE_ROOT) across persist/mod.rs, persist/markdown.rs, tests_persist.rs
    // and this file, and `cognitive_memory` excludes every test that reads/writes
    // SIMARD_STATE_ROOT (the goal-board handlers, subagent registry, …) so none runs
    // concurrently (issues #2316, #4520).
    unsafe {
        std::env::remove_var("SIMARD_MEETINGS_DIR");
