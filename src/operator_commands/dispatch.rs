...
use super::{
    ...
    run_gastronome_probe, run_goal_curation_probe, run_gym_compare, run_gym_list, run_gym_scenario,
    run_gym_suite, run_handoff_probe, run_improvement_curation_probe,
    ...
    I: IntoIterator<Item = String>,
{
    ...
    match command.as_str() {
        ...
        "gastronome-run" => {
            ...
            let brief = next_required(&mut args, "brief")?;
            reject_extra_args(args)?;
            run_gastronome_probe(&topology, &brief)?;
        }
        "handoff-roundtrip" => {
            let identity = next_required(&mut args, "identity")?;
            let base_type = next_required(&mut args, "base type")?;
...
pub fn dispatch_probe_with_context(
    ...
    match command {
        ...
        "gastronome-run" => {
            let brief = ctx.require_objective()?;
            run_gastronome_probe(&ctx.topology, brief)?;
        }
        other => return Err(format!("unsupported probe command '{other}'").into()),
    }
