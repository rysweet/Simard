...
pub fn build_greeting_banner(memory: Option<&dyn CognitiveMemoryOps>) -> Vec<String> {
    ...
    if let Some(memory) = memory {
        let projects = known_projects(memory);
        ...
        match load_goal_board(memory) {
            ...
            Ok(_) => {
                ...
                append_memory_stats(memory, &mut lines);
            }
            Err(_) => {
                append_memory_stats(memory, &mut lines);
            }
        ...
        lines.push("  Known projects: (no memory memory)".to_string());
        lines.push("  Goals: (no memory memory)".to_string());
    }
...
pub fn print_greeting_banner(memory: Option<&dyn CognitiveMemoryOps>) {
    let lines = build_greeting_banner(memory);
...
fn known_projects(memory: &dyn CognitiveMemoryOps) -> Vec<(String, f64)> {
    match memory.search_facts("project", 20, 0.0) {
        Ok(facts) => {
...
fn append_memory_stats(memory: &dyn CognitiveMemoryOps, lines: &mut Vec<String>) {
...
mod tests {
    ...
    #[test]
    fn banner_without_memory_shows_no_memory() {
        let lines = build_greeting_banner(None);
        assert!(lines.iter().any(|l| l.contains("no memory memory")));
    }
    ...
    #[test]
    fn known_projects_returns_vec_with_no_memory() {
        // Direct test of the empty case — no memory available
        let lines = build_greeting_banner(None);
        assert!(lines.iter().any(|l| l.contains("no memory memory")));
    }
    ...
    #[test]
    fn banner_no_memory_mentions_projects_and_goals() {
        let lines = build_greeting_banner(None);
    ...
    #[test]
    fn banner_is_deterministic_for_no_memory() {
