...
//! Rust execution. The Python subprocess fallback was removed in #2181.
//!
...
/// Both use in-process Rust transports (no Python subprocess).
...
mod tests {
    ...
    fn default_circuit_breaker_has_30s_cooldown() {
        let config = default_circuit_breaker();
        assert_eq!(config.cooldown, std::time::Duration::from_secs(30));
    }
