...
mod tests {
    ...
    fn builtin_loader_loads_atelier_identity() {
        ...
        assert_eq!(
            ...
            std::path::Path::new("simard/atelier_system.md")
        );
        // Atelier drives fabrication tooling from a shell, so it must carry the
        // terminal-shell base type like the engineer identity.
