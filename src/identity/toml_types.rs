...
"#;
    ...
    // F6: TomlIdentity and TomlPromptAsset must reject unknown fields
    // (deny_unknown_fields). These tests define the required behavior —
    // they will FAIL until #[serde(deny_unknown_fields)] is added.
...
"#;
        ...
        assert!(
            result.is_err(),
            "unknown field in [[identities]] should be rejected (deny_unknown_fields)"
        );
    }
...
"#;
        ...
        assert!(
            result.is_err(),
            "unknown field in prompt_assets should be rejected (deny_unknown_fields)"
        );
    }
