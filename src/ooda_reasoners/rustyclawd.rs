...
use super::{ActReasoner, EngineerLifecycleCtx, EngineerLifecycleDecision};
...
/// LLM-backed brain. Construct via `build_act_reasoner` in production so
...
impl<S: LlmSubmitter> ActReasoner for RustyClawdActReasoner<S> {
...
/// `Arc<Mutex<Box<dyn BaseTypeSession>>>` through `OodaContext`. If profiling
...
/// returns a [`RustyClawdActReasoner`] wired to a [`SessionLlmSubmitter`].
///
...
/// `DeterministicFallbackActReasoner` so the daemon behaves identically to the
...
pub fn build_act_reasoner() -> SimardResult<Box<dyn ActReasoner>> {
    let provider = LlmProvider::resolve()?;
    let submitter = SessionLlmSubmitter::new(provider);
    Ok(Box::new(RustyClawdActReasoner::new(submitter)))
}

// ---------------------------------------------------------------------------
// Inline tests (issue #1979 — per-source-file coverage of the RustyClawd
// adapter: the prose-first marker parser, the legacy JSON-object salvage,
// the UTF-8-safe log truncation, AND the adapter's end-to-end behaviour for
...
// through the adapter with a canned-response submitter).
//
// Sibling `tests.rs` covers higher-level dispatch; these inline tests pin
// the private parser helpers that the adapter depends on. )
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    // ----- (d) RustyClawdActReasoner adapter: end-to-end with stub submitter ---

    #[test]
    fn adapter_returns_decision_on_marker_response() {
        let stub = StubSubmitter::ok("DECISION: continue_skipping\nhb ok");
        let brain = RustyClawdActReasoner::new(stub);
    ...
    #[test]
    fn adapter_rejects_json_only_response() {
    ...
    #[test]
    fn adapter_rejects_json_in_prose_response() {
    ...
    #[test]
    fn adapter_surfaces_adapter_error_on_unparseable_response() {
    ...
    #[test]
    fn adapter_propagates_submitter_error_without_panic() {
    ...
    #[test]
    fn adapter_calls_submitter_exactly_once_per_decision() {
        let stub = StubSubmitter::ok("DECISION: continue_skipping\nok");
        ...
        assert_eq!(
            ...
            1,
            "adapter must call submitter exactly once per decision"
        );
    }

    #[test]
    fn adapter_renders_prompt_with_context_fields() {
        let stub = StubSubmitter::ok("DECISION: continue_skipping\nok");
        let brain = RustyClawdActReasoner::new(stub);
    ...
    #[test]
    fn adapter_renders_sentinel_none_as_placeholder() {
        let stub = StubSubmitter::ok("DECISION: continue_skipping\nok");
        let brain = RustyClawdActReasoner::new(stub);
        let prompt = brain.render_prompt(&EngineerLifecycleCtx {
            sentinel_pid: None,
