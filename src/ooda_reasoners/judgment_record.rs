...
//! deterministic floor); each call site pushes a [`ReasonerJudgmentRecord`]
...
impl ReasonerPhase {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ReasonerPhase::Act => "act",
            ReasonerPhase::Decide => "decide",
            ReasonerPhase::Orient => "orient",
            ReasonerPhase::MergeJudge => "merge_judge",
        }
    }
}
...
pub struct ReasonerJudgmentRecord {
    ...
    /// judgment (see [`crate::ooda_reasoners::prompt_store::prompt_version`]).
...
tokio::task_local! {
    ...
    static BRAIN_JUDGMENTS: RefCell<Vec<ReasonerJudgmentRecord>>;
}
...
pub fn push(record: ReasonerJudgmentRecord) {
...
/// outside a scope.
pub fn take_all() -> Vec<ReasonerJudgmentRecord> {
    BRAIN_JUDGMENTS
...
/// `clear_reasoner_judgments()` re-export. No-ops outside a scope.
pub fn clear() {
...
mod tests {
    use super::*;
    use crate::ooda_reasoners::{DecideJudgment, EngineerLifecycleDecision, OrientJudgment};
    ...
    fn record_round_trips_through_json() {
        let rec = ReasonerJudgmentRecord {
            phase: ReasonerPhase::Decide,
        ...
        let back: ReasonerJudgmentRecord = serde_json::from_str(&json).unwrap();
        assert_eq!(rec, back);
    }
    ...
    /// Byte-stability pin guarding the Step 8 `ReasonerPhase` snake_case
    ...
    fn brainphase_existing_variants_serialize_byte_stable() {
        assert_eq!(
            serde_json::to_string(&ReasonerPhase::Act).unwrap(),
            "\"act\""
        );
        assert_eq!(
            serde_json::to_string(&ReasonerPhase::Decide).unwrap(),
            "\"decide\""
        );
        assert_eq!(
            serde_json::to_string(&ReasonerPhase::Orient).unwrap(),
            "\"orient\""
        );
        // Round-trip must also stay stable.
        let back: ReasonerPhase = serde_json::from_str("\"decide\"").unwrap();
        assert_eq!(back, ReasonerPhase::Decide);
    }
    ...
    fn brainphase_merge_judge_serializes_snake_case_and_as_str_agrees() {
        assert_eq!(
            serde_json::to_string(&ReasonerPhase::MergeJudge).unwrap(),
            "\"merge_judge\""
        );
        let back: ReasonerPhase = serde_json::from_str("\"merge_judge\"").unwrap();
        assert_eq!(back, ReasonerPhase::MergeJudge);
    ...
    fn from_engineer_lifecycle_picks_variant_label() {
        ...
        let rec =
            ReasonerJudgmentRecord::from_engineer_lifecycle("g1", &dec, false, "abc123def456");
        assert_eq!(rec.phase, ReasonerPhase::Act);
    ...
    fn from_decide_uses_judgment_rationale() {
        ...
        let rec = ReasonerJudgmentRecord::from_decide("__improvement__", 0.7, &j, true, "");
    ...
    fn from_orient_labels_demotion_vs_no_op() {
        ...
        let rec = ReasonerJudgmentRecord::from_orient("g1", 0.6, 1, &demoted, false, "v1");
        ...
        let rec = ReasonerJudgmentRecord::from_orient("g1", 0.6, 0, &untouched, false, "");
    ...
    fn truncate_long_context_summary() {
        ...
        let rec = ReasonerJudgmentRecord::from_engineer_lifecycle(&long, &dec, false, "");
    ...
    fn survives_multi_thread_runtime_with_yields() {
        ...
        let collected: Vec<ReasonerJudgmentRecord> = rt.block_on(with_cycle_scope_async(async {
            for i in 0..3 {
