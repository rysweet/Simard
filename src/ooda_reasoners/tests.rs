...
//!
//! These tests define the behavioral contract of `super` (the `ooda_reasoners`
...
use super::{
    ActReasoner, DeterministicFallbackActReasoner, EngineerLifecycleCtx, EngineerLifecycleDecision,
    LlmSubmitter, RustyClawdActReasoner, gather_engineer_lifecycle_ctx, redact_secrets,
};
...
fn fallback_brain_always_continues() {
    let brain = DeterministicFallbackActReasoner;
    let decision = brain
        .decide_engineer_lifecycle(&sample_ctx())
...
fn t15_multibyte_utf8_response_does_not_panic() {
    ...
    let brain2 = RustyClawdActReasoner::new(stub2);
    let err = brain2
        .decide_engineer_lifecycle(&sample_ctx())
