...
//!   - `fallback.rs`— `DeterministicFallbackActReasoner` (preserves today's behavior).
//!   - `rustyclawd.rs` — `RustyClawdActReasoner` + `LlmSubmitter` + `build_act_reasoner`.
...
pub use decide::{
    DecideContext, DecideJudgment, DecideReasoner, DeterministicFallbackDecideReasoner,
    PROMPT_NAME as DECIDE_PROMPT_NAME,
};
pub use fallback::DeterministicFallbackActReasoner;
pub use judgment_record::{
    ReasonerJudgmentRecord, ReasonerPhase, clear as clear_reasoner_judgments,
    push as push_brain_judgment, take_all as take_reasoner_judgments,
    with_cycle_scope as with_brain_judgment_scope,
};
pub use orient::{
    DeterministicFallbackOrientReasoner, FAILURE_PENALTY_PER_CONSECUTIVE, OrientContext,
    OrientJudgment, OrientReasoner, PROMPT_NAME as ORIENT_PROMPT_NAME, RustyClawdOrientReasoner,
    build_orient_reasoner,
};
pub use parse_failure::ParseFailureRecord;
pub use recipe_reasoner::RecipeReasoner;
...
pub type RecipeDecideReasoner = RecipeReasoner;
pub type RecipeEngineerLifecycleReasoner = RecipeReasoner;
pub type RecipeOrientReasoner = RecipeReasoner;
pub use rustyclawd::{
    LlmSubmitter, PROMPT_NAME as ACT_PROMPT_NAME, RustyClawdActReasoner, SessionLlmSubmitter,
    build_act_reasoner,
};
...
/// sync, and the LLM-backed impl adapters to async internally so callers do
...
// `ReasonerPhase` serde round-trip that the `parse_failure::counters()` map
...
mod inline_tests_1979 {
    ...
    // ----- ReasonerPhase serde round-trip -----------------------------------
    // The map `parse_failure::counters(): (ReasonerPhase, goal_id) -> u32` keys
    // on `ReasonerPhase`. An incorrect Hash/Eq/serde impl silently re-buckets
    ...
    fn brain_phase_round_trips_through_json() {
        for &phase in &[
            ...
            let back: ReasonerPhase = serde_json::from_str(&s).unwrap();
    ...
    fn brain_phase_distinct_variants_are_not_equal() {
        ...
        assert_ne!(ReasonerPhase::Act, ReasonerPhase::Decide);
        assert_ne!(ReasonerPhase::Decide, ReasonerPhase::Orient);
        assert_ne!(ReasonerPhase::Act, ReasonerPhase::Orient);
    }
