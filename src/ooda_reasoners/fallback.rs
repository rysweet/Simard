...
impl ActReasoner for DeterministicFallbackActReasoner {
...
// that consumers depend on when the LLM adapter returns unparseable JSON or
...
mod tests {
    ...
    fn fallback_is_deterministic_across_varied_contexts() {
        ...
        // JSON-parse failure in the LLM adapter).
        let brain = DeterministicFallbackActReasoner;
        let contexts = [
            EngineerLifecycleCtx {
                failure_count: 0,
