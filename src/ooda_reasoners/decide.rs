...
//! [`super::recipe_decide::RecipeDecideReasoner`] (recipe-runner-rs subprocess).
...
//! availability for Decide: [`DeterministicFallbackDecideReasoner`] preserves the
...
/// [`super::ActReasoner`] — the LLM-backed impl adapters to async internally so
...
pub struct DeterministicFallbackDecideReasoner;

impl DecideReasoner for DeterministicFallbackDecideReasoner {
    fn judge_decision(&self, ctx: &DecideContext) -> SimardResult<DecideJudgment> {
