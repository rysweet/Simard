...
//!
//!   1. `tracing::error!` (target `simard::ooda_reasoners`) with structured fields.
...
//!      `ReasonerJudgmentRecord.parse_failure` field that lands in
...
/// One brain-invocation failure record, embedded on `ReasonerJudgmentRecord`.
///
...
pub struct ParseFailureRecord {
    /// `"decide"` or `"orient"`. Lowercased to match `ReasonerPhase` serde.
    pub phase: String,
...
fn counters() -> &'static Mutex<HashMap<(ReasonerPhase, String), u32>> {
    static CTRS: OnceLock<Mutex<HashMap<(ReasonerPhase, String), u32>>> = OnceLock::new();
...
pub(crate) fn bump_consecutive_count(phase: ReasonerPhase, goal_id: &str) -> u32 {
...
pub(crate) fn reset_consecutive_count(phase: ReasonerPhase, goal_id: &str) {
    let mut guard = counters()
        .lock()
        .expect("parse_failure counter mutex poisoned");
...
#[cfg(test)]
pub(crate) fn peek_consecutive_count(phase: ReasonerPhase, goal_id: &str) -> u32 {
    let guard = counters()
        .lock()
        .expect("parse_failure counter mutex poisoned");
    guard
...
#[cfg(test)]
pub(crate) fn reset_consecutive_count_for_tests(phase: ReasonerPhase, goal_id: &str) {
...
///      aggregators (target `simard::ooda_reasoners`).
...
///      `ReasonerJudgmentRecord` that lands in `cycle_*.json`.
...
pub(crate) fn record_parse_failure(
    ...
    // `target=simard::ooda_reasoners` to surface this.
    tracing::error!(
...
fn maybe_escalate_to_gh_issue(record: &ParseFailureRecord) {
    ...
    let body = format!(
         ...
         **Timestamp:** {}\n\n\
         Inspect the brain logs (`journalctl -u simard | grep simard::ooda_reasoners`) and \
         the prompt asset before reactivating. See \
         `docs/howto/diagnose-decide-orient-parse-failures.md` for the operator runbook.\n\n\
         Filed deterministically by `src/ooda_reasoners/parse_failure.rs` (issue #1890).\n\
...
fn phase_to_string(phase: ReasonerPhase) -> String {
...
mod tests {
    ...
    fn consecutive_count_increments_per_phase_goal_id_pair() {
        ...
        let r1 = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
        let r2 = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
        let r3 = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
    ...
    fn consecutive_count_tracked_separately_per_phase() {
        ...
        let d = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
        let o = record_parse_failure(ReasonerPhase::Orient, goal, &err, "x", "p", String::new());
    ...
    fn reset_consecutive_count_clears_counter_for_pair() {
        ...
        let _ = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
        let _ = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
        assert_eq!(peek_consecutive_count(ReasonerPhase::Decide, goal), 2);
        reset_consecutive_count(ReasonerPhase::Decide, goal);
        assert_eq!(peek_consecutive_count(ReasonerPhase::Decide, goal), 0);
        // Next failure starts fresh at 1.
        let next = record_parse_failure(ReasonerPhase::Decide, goal, &err, "x", "p", String::new());
    ...
    fn reset_consecutive_count_for_one_pair_does_not_affect_others() {
        ...
        reset_consecutive_count(ReasonerPhase::Decide, "reset-isolation-A");
        assert_eq!(
            peek_consecutive_count(ReasonerPhase::Decide, "reset-isolation-A"),
            0
        );
        assert_eq!(
            peek_consecutive_count(ReasonerPhase::Decide, "reset-isolation-B"),
            1
        );
    }
