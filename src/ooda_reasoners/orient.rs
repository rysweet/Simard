...
//! availability for Orient: [`DeterministicFallbackOrientReasoner`] preserves
...
/// [`super::ActReasoner`] and [`super::DecideReasoner`] — the LLM-backed impl
/// adapters to async internally so callers do not need a runtime.
...
pub struct DeterministicFallbackOrientReasoner;

impl DeterministicFallbackOrientReasoner {
...
impl OrientReasoner for DeterministicFallbackOrientReasoner {
    fn judge_orientation(&self, ctx: &OrientContext) -> SimardResult<OrientJudgment> {
        Ok(Self::compute(ctx))
    }
}
...
// ---------------------------------------------------------------------------
// LLM-backed brain (mirrors RustyClawdDecideReasoner shape from PR #1469)
// ---------------------------------------------------------------------------
...
/// uses [`build_orient_reasoner`] to construct one wired to a real
/// session.
pub struct RustyClawdOrientReasoner<S: LlmSubmitter> {
    submitter: S,
}

impl<S: LlmSubmitter> RustyClawdOrientReasoner<S> {
...
impl<S: LlmSubmitter> OrientReasoner for RustyClawdOrientReasoner<S> {
    fn judge_orientation(&self, ctx: &OrientContext) -> SimardResult<OrientJudgment> {
