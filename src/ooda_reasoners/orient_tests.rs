...
use super::{
    DeterministicFallbackOrientReasoner, FAILURE_PENALTY_PER_CONSECUTIVE, LlmSubmitter,
    OrientContext, OrientJudgment, OrientReasoner, RustyClawdOrientReasoner,
};
...
// ---------------------------------------------------------------------------
// DeterministicFallbackOrientReasoner — preserves pre-#1469 formula bit-for-bit
// ---------------------------------------------------------------------------
...
fn fallback_judgment_passes_validate() {
    ...
    let j = DeterministicFallbackOrientReasoner::compute(&context);
    j.validate(context.base_urgency).expect("must validate");
}

// ---------------------------------------------------------------------------
// RustyClawdOrientReasoner — round-trip via stub submitter
// ---------------------------------------------------------------------------
...
fn rustyclawd_brain_parses_canned_response() {
    ...
    let brain = RustyClawdOrientReasoner::new(stub);
    let j = brain.judge_orientation(&ctx(1, 0.8)).unwrap();
...
fn trait_object_compiles_for_both_impls() {
    ...
    let brains: Vec<Box<dyn OrientReasoner>> = vec![
        Box::new(DeterministicFallbackOrientReasoner),
        Box::new(RustyClawdOrientReasoner::new(stub)),
    ];
    for b in &brains {
