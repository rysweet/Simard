...
//! [`NativeRpcTransport`], delegating the actual evaluation to
...
use crate::rpc::{RPC_ERROR_INTERNAL, RpcErrorPayload};
...
///
/// This is the sole entry point `rpc_subprocess_launcher::launch_gym_client_native`
...
pub fn register_gym_handlers(transport: &mut NativeRpcTransport) {
    transport.register(
        "gym.list_scenarios",
        Arc::new(|_params: &Value| -> Result<Value, RpcErrorPayload> {
    ...
    transport.register(
        "gym.run_scenario",
        Arc::new(|params: &Value| -> Result<Value, RpcErrorPayload> {
...
mod tests {
    use super::*;
    use crate::rpc::{RPC_ERROR_INTERNAL, RpcRequest, RpcResponse, RpcTransport, new_request_id};
    use serde_json::json;
    use serial_test::serial;

    // ── Test plumbing ────────────────────────────────────────────────────

    fn make_transport() -> NativeRpcTransport {
    ...
    // ── RPC health (unchanged transport behaviour) ─────────────────────
