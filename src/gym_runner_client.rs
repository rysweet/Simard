...
//!
//! Thin Simard adapter that backs the `simard-gym-eval` adapter with the
...
//! [`NativeServerTransport`], delegating the actual evaluation to
...
//! library; this module is *only* the adapter wiring.
//!
...
use crate::server_subprocess::NativeServerTransport;
use crate::server_transport::{SERVER_ERROR_INTERNAL, ServerErrorPayload};
...
/// [`ScoreDimensions`] fields the adapter wire contract requires.
///
...
/// Map a library [`GymScenarioResult`] onto the adapter wire JSON, overriding
...
///
/// This is the sole entry point `server_launcher::launch_gym_client_native`
...
pub fn register_gym_handlers(transport: &mut NativeServerTransport) {
    transport.register(
        "gym.list_scenarios",
        Arc::new(|_params: &Value| -> Result<Value, ServerErrorPayload> {
    ...
    transport.register(
        "gym.run_scenario",
        Arc::new(|params: &Value| -> Result<Value, ServerErrorPayload> {
    ...
    transport.register(
        ...
        Arc::new(|params: &Value| -> Result<Value, ServerErrorPayload> {
            ...
            // traversal check of its own, so the adapter must validate it here.
...
mod tests {
    ...
    use crate::server_transport::{
        SERVER_ERROR_INTERNAL, ServerRequest, ServerResponse, ServerTransport, new_request_id,
    };
    use serde_json::json;
    ...
    fn make_transport() -> NativeServerTransport {
        let mut transport = NativeServerTransport::new("simard-gym-eval");
        register_gym_handlers(&mut transport);
        transport
    }

    /// Issue an adapter call and return the raw response (so error envelopes are
    /// inspectable).
    fn raw_call(transport: &NativeServerTransport, method: &str, params: Value) -> ServerResponse {
    ...
    /// Issue an adapter call and return the `result` payload, asserting success.
    fn result_of(transport: &NativeServerTransport, method: &str, params: Value) -> Value {
    ...
    fn run_suite_rejects_path_traversal() {
        ...
        // traversal check; the adapter must reject it itself.
        let _g = SkipGuard::clear();
        let transport = make_transport();
        let r = result_of(
            &transport,
    ...
    fn run_scenario_empty_id_is_transport_error() {
        ...
        assert_eq!(err.code, SERVER_ERROR_INTERNAL);
    }
    ...
    fn skip_result_deserializes_as_gym_scenario_result() {
        ...
        let typed: crate::gym_client::GymScenarioResult = serde_json::from_value(r)
    ...
    fn skip_suite_result_deserializes_as_gym_suite_result() {
        ...
        let typed: crate::gym_client::GymSuiteResult = serde_json::from_value(r)
            .expect("handler JSON must deserialize into the unchanged GymSuiteResult");
        assert!(typed.success);
        assert_eq!(typed.scenarios_total, 0);
    }

    // ── Adapter health (unchanged transport behaviour) ─────────────────────
