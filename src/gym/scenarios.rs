...
const BENCHMARK_SCENARIOS: [BenchmarkScenario; 148] = [
    BenchmarkScenario {
    ...
    // --- Wave 10: deepen coverage for under-represented classes (issue #1177) ---
    BenchmarkScenario {
        id: "a11y-form-validation-multiprocess-rusty-clawd",
        title: "Accessibility review: form validation messaging on multi-process rusty-clawd",
        description: "Audit form validation error surfacing for accessible programmatic association and live-region updates through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::AccessibilityReview,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit form error reporting for accessibility. Cover: (1) aria-invalid and aria-describedby wiring between input and error message, (2) aria-live polite/assertive regions for asynchronous server validation, (3) focus management when a submission fails, (4) WCAG 3.3.1, 3.3.3, and 4.1.3. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "a11y-screen-reader-walkthrough-distributed-copilot",
        title: "Accessibility review: screen reader walkthrough on distributed copilot-sdk",
        description: "Walk an end-to-end task with a screen reader and document the experience for major SR/browser combinations through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::AccessibilityReview,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Document a screen reader walkthrough of a critical task. Cover: (1) tested combinations (NVDA+Firefox, JAWS+Chrome, VoiceOver+Safari), (2) heading/landmark navigation effectiveness, (3) dynamic content announcements and dialog focus trapping, (4) WCAG 1.3.1, 2.4.6, and 4.1.2 findings with concrete remediations. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "api-design-pagination-multiprocess-copilot",
        title: "API design: pagination contract on multi-process copilot-sdk",
        description: "Design a forward-compatible pagination contract for a list endpoint through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ApiDesign,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design pagination for a list endpoint. Cover: (1) cursor vs offset trade-offs and the chosen scheme, (2) request parameters (limit, cursor) and stable-sort guarantees, (3) response shape (items, next_cursor, has_more) and end-of-stream signal, (4) deprecation strategy for an existing offset-based version. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "api-design-error-shape-distributed-terminal",
        title: "API design: error response shape on distributed terminal-shell",
        description: "Design a uniform machine-readable error envelope across services through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::ApiDesign,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Design a uniform error envelope. Cover: (1) RFC 7807 problem+json fields (type, title, status, detail, instance) and any extensions, (2) stable error code taxonomy and namespacing, (3) retryability hints and rate-limit metadata, (4) backward-compatibility rules for adding new fields. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "caching-write-through-multiprocess-rusty-clawd",
        title: "Caching strategy: write-through layer on multi-process rusty-clawd",
        description: "Design a write-through cache between the API and persistent store through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::CachingStrategy,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design a write-through cache. Cover: (1) cache key shape and namespacing, (2) write ordering (cache then store vs store then cache) and failure handling, (3) read-path miss handling and stampede protection (single-flight, jitter), (4) consistency guarantees and invalidation triggers. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "caching-tiered-eviction-distributed-copilot",
        title: "Caching strategy: tiered eviction on distributed copilot-sdk",
        description: "Plan a multi-tier (in-process LRU + distributed cache) layout with coordinated eviction through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::CachingStrategy,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Plan a tiered cache. Cover: (1) sizing and TTL per tier (L1 in-process LRU vs L2 distributed), (2) eviction policy and coordinated invalidation (pub/sub or versioned keys), (3) hit-rate and tail-latency measurement plan, (4) failure modes when L2 is unreachable and graceful degradation. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "chaos-cpu-saturation-multiprocess-copilot",
        title: "Chaos engineering: CPU saturation experiment on multi-process copilot-sdk",
        description: "Design a controlled CPU saturation experiment on a single instance through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ChaosEngineering,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design a CPU saturation experiment. Cover: (1) injection mechanism (stress-ng, taskset pinning, or in-process busy loop) and ramp pattern, (2) steady-state hypothesis (latency p99 within X ms) and abort signal (error rate > Y%), (3) blast-radius controls (single instance, time-boxed, automatic rollback), (4) telemetry needed to attribute regressions to the injection. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "chaos-dependency-blackhole-distributed-terminal",
        title: "Chaos engineering: dependency black-hole on distributed terminal-shell",
        description: "Design an experiment that black-holes traffic to a downstream dependency through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::ChaosEngineering,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Design a dependency black-hole experiment. Cover: (1) the targeted dependency and the network mechanism (iptables drop, sidecar reject, DNS), (2) hypothesis about circuit-breaker / fallback behavior, (3) blast-radius scoping to a non-prod cell or canary fraction, (4) recovery validation (steady-state restored within RTO). Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "cicd-pr-gating-multiprocess-rusty-clawd",
        title: "CI/CD pipeline: pull-request gating on multi-process rusty-clawd",
        description: "Design the required-checks gating policy for pull requests through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::CicdPipeline,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design PR gating. Cover: (1) required status checks (build, lint, unit tests, security scan) and their order, (2) parallelism, caching, and target wall-clock budget, (3) flaky-test handling (auto-retry policy, quarantine), (4) override path for hotfixes and the audit record it produces. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "cicd-canary-promotion-distributed-copilot",
        title: "CI/CD pipeline: canary promotion gates on distributed copilot-sdk",
        description: "Design automated canary promotion gates from staging to production through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::CicdPipeline,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Design canary promotion. Cover: (1) traffic split steps (1% -> 10% -> 50% -> 100%) and bake time per step, (2) automated metric checks per step (error rate, latency, business KPI) with explicit thresholds, (3) automatic rollback trigger and manual hold path, (4) cross-region rollout ordering. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "concurrency-deadlock-audit-multiprocess-copilot",
        title: "Concurrency analysis: deadlock audit on multi-process copilot-sdk",
        description: "Audit a critical-section graph for potential deadlocks and lock-ordering inversions through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ConcurrencyAnalysis,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit for deadlock risk. Cover: (1) inventory of locks and the order in which they are acquired on each code path, (2) at least one path that demonstrates safe ordering and one site that risks inversion, (3) static or runtime tooling that could detect violations (e.g., lockdep-style ordering invariants), (4) proposed remediation (lock hierarchy, try-lock with backoff, or finer-grained locks). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "concurrency-async-cancellation-distributed-terminal",
        title: "Concurrency analysis: async cancellation safety on distributed terminal-shell",
        description: "Audit async cancellation propagation and resource cleanup on a long-lived task graph through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::ConcurrencyAnalysis,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Audit async cancellation. Cover: (1) sites that .await across resource-holding boundaries (file handles, locks, transactions), (2) cancellation propagation strategy (CancellationToken or Drop-based) and whether partial work is left behind, (3) at least one structured-concurrency pattern that fixes a leak, (4) test approach that exercises drop at every await point. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "schema-add-column-backfill-multiprocess-rusty-clawd",
        title: "Database schema change: add column with online backfill on multi-process rusty-clawd",
        description: "Design an online add-column-and-backfill migration with zero-downtime guarantees through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::DatabaseSchemaChange,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design an online add-column migration. Cover: (1) DDL statement and lock implications (e.g., adding nullable, no default), (2) chunked backfill plan with rate limit and progress tracking, (3) application-side rollout order (deploy reader -> backfill -> deploy writer), (4) rollback at each phase. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "schema-drop-column-deprecation-distributed-copilot",
        title: "Database schema change: column deprecation on distributed copilot-sdk",
        description: "Plan a multi-phase column deprecation across services through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::DatabaseSchemaChange,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Plan a column drop. Cover: (1) consumer inventory and stop-write rollout sequence across services, (2) shadow-read verification window before the drop, (3) DDL with concurrent-safe drop and replication considerations, (4) reversibility window and backup strategy before the destructive step. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "debug-panic-backtrace-multiprocess-copilot",
        title: "Debugging: panic backtrace triage on multi-process copilot-sdk",
        description: "Triage a captured panic backtrace, localize the failing site, and propose a fix through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::Debugging,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Triage a panic. Cover: (1) interpret the backtrace and identify the originating function and likely invariant violated, (2) hypothesize at least two upstream causes consistent with the trace, (3) propose a minimal repro test that fails today and passes after the fix, (4) describe the smallest correct change with rationale. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "debug-flaky-test-bisect-distributed-terminal",
        title: "Debugging: flaky test bisect on distributed terminal-shell",
        description: "Investigate a flaky test, isolate the non-determinism, and propose a stabilization plan through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::Debugging,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Stabilize a flaky test. Cover: (1) reproduction strategy (loop count, parallelism, --test-threads) that surfaces the flake, (2) suspected non-determinism source (time, ordering, network, shared state) with evidence, (3) targeted fix (mock clock, deterministic seed, isolate state) and a regression guard, (4) classify whether the flake masks a real bug. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-analysis-license-audit-multiprocess-rusty-clawd",
        title: "Dependency analysis: license audit on multi-process rusty-clawd",
        description: "Run a license audit across the dependency graph and surface incompatibilities through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::DependencyAnalysis,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit licenses. Cover: (1) tooling (cargo-deny / cargo-license) and the policy file checked in, (2) classification of each direct dependency by license family (permissive / weak copyleft / strong copyleft / proprietary), (3) at least one example violation or risk and its remediation, (4) recurrence prevention (CI gate, allowlist updates). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-analysis-supply-chain-distributed-copilot",
        title: "Dependency analysis: supply-chain provenance on distributed copilot-sdk",
        description: "Assess supply-chain provenance and pinning strategy across the dependency graph through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::DependencyAnalysis,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Assess supply-chain risk. Cover: (1) Cargo.lock pinning posture and reproducible-build implications, (2) source verification (registry index integrity, git-source pinning by commit), (3) SBOM generation (CycloneDX or SPDX) and where it is published, (4) advisory monitoring (cargo-audit, RustSec) and triage SLA. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-upgrade-tokio-major-multiprocess-copilot",
        title: "Dependency upgrade: tokio major version on multi-process copilot-sdk",
        description: "Plan a tokio major-version bump with API and runtime-behavior diffs through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::DependencyUpgrade,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan a tokio major upgrade. Cover: (1) breaking-change inventory from the upstream changelog and impacted call sites, (2) staged migration (feature-flagged compatibility shims if needed), (3) async-runtime behavior diffs (scheduler, IO driver) and bench plan, (4) rollback strategy if regressions appear. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-upgrade-serde-edition-distributed-terminal",
        title: "Dependency upgrade: serde derive edition bump on distributed terminal-shell",
        description: "Coordinate a serde + serde_derive edition bump across crates through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::DependencyUpgrade,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Coordinate a cross-crate serde upgrade. Cover: (1) workspace lockstep strategy (single Cargo.lock, version unification, [patch] usage if any), (2) on-the-wire compatibility check for serialized payloads, (3) deny-warnings posture for new lints introduced by the upgrade, (4) phased rollout across services. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "doc-architecture-decision-record-multiprocess-rusty-clawd",
        title: "Documentation: architecture decision record on multi-process rusty-clawd",
        description: "Draft an Architecture Decision Record for an active design choice through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::Documentation,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Draft an ADR. Cover: (1) context and the forces in tension, (2) at least three considered options with trade-offs, (3) the chosen decision and its consequences (positive, negative, neutral), (4) status, supersession links, and review date. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "doc-runbook-onboarding-distributed-copilot",
        title: "Documentation: onboarding runbook on distributed copilot-sdk",
        description: "Author a new-engineer onboarding runbook for the runtime through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::Documentation,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Author an onboarding runbook. Cover: (1) prerequisites and bootstrap commands with expected output, (2) first-week milestones (build, run sample, ship a trivial PR), (3) where to find owners, dashboards, and the on-call schedule, (4) common pitfalls with verified workarounds. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "event-sourcing-snapshot-strategy-multiprocess-copilot",
        title: "Event sourcing: snapshot strategy on multi-process copilot-sdk",
        description: "Design a snapshot strategy for an event-sourced aggregate through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::EventSourcing,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design snapshots. Cover: (1) snapshot trigger (event count, time, or size threshold) and storage location, (2) replay path (load latest snapshot then apply tail), (3) versioning so snapshots are forward/backward compatible, (4) operational concerns (snapshot retention, GC, restore validation). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "event-sourcing-projection-rebuild-distributed-terminal",
        title: "Event sourcing: projection rebuild on distributed terminal-shell",
        description: "Plan an online projection rebuild after a schema change through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::EventSourcing,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Plan a projection rebuild. Cover: (1) shadow projection table populated alongside the live one, (2) catch-up strategy from the event log without blocking writes, (3) cutover signal (lag below threshold, parity check), (4) rollback path if the new projection misbehaves. Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "feature-flag-targeted-rollout-multiprocess-rusty-clawd",
        title: "Feature flagging: targeted percentage rollout on multi-process rusty-clawd",
        description: "Design a percentage and attribute-targeted rollout for a feature flag through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::FeatureFlagging,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design a targeted rollout. Cover: (1) bucketing function and stable hashing key, (2) attribute-based overrides (allowlist, region, tier) with precedence, (3) kill-switch and the propagation latency target, (4) telemetry to compare control vs treatment cohorts. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "feature-flag-cleanup-audit-distributed-copilot",
        title: "Feature flagging: stale-flag cleanup audit on distributed copilot-sdk",
        description: "Audit the flag inventory for stale flags and propose a removal plan through the distributed topology with the copilot-sdk base type.",
        class: BenchmarkClass::FeatureFlagging,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::Distributed,
        objective: "Audit flag inventory. Cover: (1) categorize flags (release / experiment / ops / permission) with last-eval dates, (2) identify at least three stale flags safe to remove, (3) removal procedure (default the flag, soak, delete code, drop config), (4) recurrence prevention (TTL, owner, cleanup CI check). Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "incident-response-status-comm-multiprocess-rusty-clawd",
        title: "Incident response: customer status communications on multi-process rusty-clawd",
        description: "Author the customer-facing status update sequence for a degradation through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::IncidentResponse,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Draft external status comms. Cover: (1) initial acknowledgement template within the SLA window, (2) periodic update cadence and what each update must include, (3) resolution announcement and follow-up RCA commitment, (4) tone, scope, and what to avoid (speculation, blame, internal jargon). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "i18n-message-catalog-ci-multiprocess-copilot",
        title: "Internationalization review: message catalog CI checks on multi-process copilot-sdk",
        description: "Design CI checks that keep message catalogs in sync with source through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::InternationalizationReview,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design i18n CI checks. Cover: (1) catalog-vs-source key drift detection (missing/orphan keys), (2) ICU/Fluent placeholder validation (counts and named args), (3) translator workflow integration (export to TMS, import back), (4) blocking vs warning severity policy. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "migration-strangler-fig-multiprocess-copilot",
        title: "Migration planning: strangler-fig rollout on multi-process copilot-sdk",
        description: "Plan a strangler-fig migration from a legacy module to a replacement through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::MigrationPlanning,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan a strangler-fig migration. Cover: (1) seam where new code intercepts traffic from old, (2) sequencing of capability slices to migrate, (3) parity verification (shadow traffic, response diff), (4) decommission criteria and the cleanup PR plan. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "observability-slo-error-budget-distributed-terminal",
        title: "Observability instrumentation: SLO and error budget on distributed terminal-shell",
        description: "Define an SLO, error budget, and burn-rate alerts for a critical request path through the distributed topology with the terminal-shell base type.",
        class: BenchmarkClass::ObservabilityInstrumentation,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::Distributed,
        objective: "Define an SLO. Cover: (1) precise SLI definition (availability or latency, with the measurement window), (2) target SLO and the implied 30-day error budget, (3) multi-window multi-burn-rate alert thresholds, (4) policy for budget exhaustion (freeze risky changes). Confirm the distributed topology backend appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
];
