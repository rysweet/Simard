...
const BENCHMARK_SCENARIOS: [BenchmarkScenario; 160] = [
    ...
    // --- Wave 7 expansion: bring under-represented classes from 3 to 5 scenarios each ---
    BenchmarkScenario {
        id: "a11y-screen-reader-flow-local",
        title: "Accessibility review: screen-reader flow on local harness",
        description: "Audit a screen-reader traversal of a primary user flow on the single-process local harness, focusing on landmark order and announcement quality.",
        class: BenchmarkClass::AccessibilityReview,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Audit a primary user flow for screen-reader usability. Cover: (1) landmark and heading order so the user can skim the page non-visually, (2) accessible names for all interactive controls (buttons, links, form fields) including dynamic state announcements, (3) live-region usage for async status updates with appropriate politeness, (4) at least three concrete WCAG 2.1 AA citations that justify the findings.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "a11y-focus-trap-multiprocess-rusty-clawd",
        title: "Accessibility review: modal focus trap on multi-process rusty-clawd",
        description: "Verify focus management and trap correctness for modal dialogs through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::AccessibilityReview,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit modal dialog focus behavior. Cover: (1) initial focus placement when the modal opens and the rationale, (2) tab and shift-tab cycling staying within the modal until dismissal, (3) restoring focus to the originating control on close (including escape-key handling), (4) screen-reader announcement of the modal role and accessible name. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "api-design-pagination-local",
        title: "API design: cursor pagination on local harness",
        description: "Design opaque cursor-based pagination for a list endpoint on the single-process local harness.",
        class: BenchmarkClass::ApiDesign,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Design cursor pagination for a list endpoint. Cover: (1) opaque base64 cursor encoding the sort key plus tiebreaker (e.g., (created_at, id)) so insertions during paging do not skip or duplicate rows, (2) page-size parameter with documented upper bound and default, (3) response shape including next_cursor and a has_more boolean, (4) behavior when the cursor is malformed or stale (HTTP status and error envelope).",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "api-design-versioning-multiprocess-copilot",
        title: "API design: versioning strategy on multi-process copilot-sdk",
        description: "Choose and justify an API versioning strategy that supports breaking changes through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ApiDesign,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design an API versioning strategy. Cover: (1) chosen mechanism (URI prefix vs Accept-header vs query parameter) and the trade-offs, (2) deprecation policy with minimum support window and Sunset/Deprecation header usage, (3) migration story for clients including a side-by-side period and observable adoption metrics, (4) a concrete example of one breaking change shipped through the versioning scheme. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "caching-cache-stampede-local",
        title: "Caching strategy: stampede mitigation on local harness",
        description: "Design a cache-stampede mitigation for a hot key under sudden load on the single-process local harness.",
        class: BenchmarkClass::CachingStrategy,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Design cache-stampede protection. Cover: (1) the chosen technique (single-flight coalescing, request lock, or probabilistic early refresh) and why, (2) staleness bound that limits how long a serving thread may return a slightly stale value while a refresh runs, (3) failure-mode behavior when the upstream is slow or down (negative caching with a short TTL, or fall-through with a circuit breaker), (4) at least one concrete numeric example showing the request count to the upstream during a stampede event before and after the mitigation.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "caching-cdn-purge-multiprocess-terminal",
        title: "Caching strategy: CDN purge on multi-process terminal-shell",
        description: "Design a safe purge procedure for cached assets at the CDN edge through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::CachingStrategy,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design a CDN purge procedure. Cover: (1) granularity of purge (single URL, surrogate-key tag, or wildcard) and the operational risk of each, (2) propagation latency budget across edge locations and how to verify completion, (3) protection against accidental mass purge (rate limit, dry-run, audit log), (4) rollback if the purge causes an origin overload (rapid re-warm or canary purge first). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "chaos-dependency-failover-local",
        title: "Chaos engineering: dependency failover drill on local harness",
        description: "Plan a controlled experiment that fails a downstream dependency and verifies graceful degradation on the single-process local harness.",
        class: BenchmarkClass::ChaosEngineering,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Plan a dependency-failover chaos drill. Cover: (1) the dependency selected, the failure mode injected (timeout, 5xx, connection refused), and the duration, (2) the expected graceful-degradation behavior (cached response, default value, partial-page render) and the user-visible impact budget, (3) the metrics that confirm degradation actually triggered (fallback counter, upstream error rate), (4) the abort signal and post-experiment cleanup including verification that fallback caches are not poisoned.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "chaos-resource-exhaustion-multiprocess-copilot",
        title: "Chaos engineering: resource-exhaustion drill on multi-process copilot-sdk",
        description: "Plan an experiment that exhausts CPU, memory, or file descriptors on a single instance through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ChaosEngineering,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan a resource-exhaustion chaos drill. Cover: (1) the resource targeted (CPU, memory, FD, disk) and the injection tool (cgroup limit, stress-ng, ulimit), (2) the expected scheduler/orchestrator response (eviction, restart, autoscaling) and the recovery budget, (3) safeguards so the experiment cannot cascade beyond a single instance (anti-affinity, pod disruption budget), (4) the post-mortem checklist for any behavior outside the predicted envelope. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "cicd-required-checks-local",
        title: "CI/CD pipeline: required-checks policy on local harness",
        description: "Define the required-checks policy for the default branch on the single-process local harness.",
        class: BenchmarkClass::CicdPipeline,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Define a required-checks policy. Cover: (1) the minimum set of checks that must pass before merge (build, lint, unit tests, security scan) and the rationale for each, (2) checks that are advisory vs blocking and the criteria for promotion from advisory to blocking, (3) the protected-branch settings that enforce the policy (linear history, dismiss stale reviews, no force-push), (4) the exception process for hotfixes including the audit trail it produces.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "cicd-deploy-canary-multiprocess-rusty-clawd",
        title: "CI/CD pipeline: canary deploy stage on multi-process rusty-clawd",
        description: "Design a canary deploy stage with automated health gates through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::CicdPipeline,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design a canary deploy stage. Cover: (1) traffic split percentages and the time each step holds before promotion, (2) automated health gates (error-rate, latency p95, saturation) that pause or roll back, (3) the rollback path including how the previous version's artifact is kept warm, (4) operator override and the audit log it produces. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "concurrency-deadlock-audit-local",
        title: "Concurrency analysis: deadlock audit on local harness",
        description: "Audit a multi-lock code path for potential deadlocks on the single-process local harness.",
        class: BenchmarkClass::ConcurrencyAnalysis,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Audit a code path that acquires multiple locks for deadlock risk. Pick a function in src/runtime/ or src/memory/ that takes more than one lock. Cover: (1) the lock acquisition order at every call site, (2) any cycle in the resulting lock-order graph, (3) a fix that establishes a global lock-ordering rule or replaces nested locks with a single coarser lock, (4) the test or invariant that would catch a regression.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "concurrency-async-cancellation-multiprocess-copilot",
        title: "Concurrency analysis: async cancellation safety on multi-process copilot-sdk",
        description: "Audit async tasks for cancellation safety under structured concurrency through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ConcurrencyAnalysis,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit async cancellation safety. Cover: (1) tasks that hold resources across .await points and the leak risk if cancelled mid-await, (2) RAII or scope-guard patterns that ensure cleanup runs even on cancellation, (3) explicit cancellation tokens vs structured-concurrency abort propagation and which the codebase uses, (4) at least one concrete example of a task that is currently not cancellation-safe with a proposed fix. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "db-schema-add-column-local",
        title: "Database schema change: add nullable column on local harness",
        description: "Plan an additive nullable-column migration with backfill on the single-process local harness.",
        class: BenchmarkClass::DatabaseSchemaChange,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Plan an additive column migration. Cover: (1) the ALTER statement and why nullable-with-default is preferred over a non-null backfill in a single transaction, (2) the backfill script, batch size, and estimated duration on production-scale data, (3) the application deploy ordering (write-old/read-old, write-both, read-new, drop-old) so reads and writes never see an inconsistent schema, (4) the rollback procedure if the backfill or app deploy fails midway.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "db-schema-rename-column-multiprocess-terminal",
        title: "Database schema change: rename column safely on multi-process terminal-shell",
        description: "Plan a zero-downtime rename of a hot column through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::DatabaseSchemaChange,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan a zero-downtime column rename. Cover: (1) the expand-and-contract sequence (add new column, dual-write, backfill, switch reads, drop old), (2) the dual-write trigger or application code path and how it handles partial failure, (3) data-consistency verification (checksum or row-count diff) before the read switch, (4) the rollback at each phase with a documented decision point. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "debug-flaky-test-local",
        title: "Debugging: flaky test triage on local harness",
        description: "Triage an intermittent test failure and propose a deterministic fix on the single-process local harness.",
        class: BenchmarkClass::Debugging,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Triage a flaky test. Pick a #[test] in src/gym/ or src/runtime/ that uses time, ordering, or shared state. Cover: (1) the suspected source of nondeterminism (clock, hashmap iteration order, async race, shared global), (2) a minimal reproduction strategy (loop the test N times, set RUST_TEST_THREADS, inject seed) that reliably surfaces the failure, (3) a deterministic fix (mock clock, sort before assert, explicit synchronization), (4) a guard test or assertion that would catch a regression of the same flakiness.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "debug-memory-growth-multiprocess-rusty-clawd",
        title: "Debugging: slow memory growth on multi-process rusty-clawd",
        description: "Investigate a process whose memory grows over hours and identify the leak through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::Debugging,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Investigate gradual memory growth. Cover: (1) the observability signal that flagged the growth (RSS trend, GC pressure, allocator stats), (2) the diagnostic procedure (heap profile sampling, retained-size analysis, comparing snapshots), (3) the candidate root causes ranked (unbounded cache, retained closure, cyclical reference), (4) the fix and the regression test (e.g., a long-running test with a memory-ceiling assertion). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-analysis-license-audit-local",
        title: "Dependency analysis: license audit on local harness",
        description: "Audit transitive dependency licenses for compliance risk on the single-process local harness.",
        class: BenchmarkClass::DependencyAnalysis,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Audit Cargo.lock for license compliance. Cover: (1) the allowlist of acceptable SPDX identifiers (e.g., MIT, Apache-2.0, BSD-3-Clause) and the rationale, (2) the procedure to scan transitive deps (cargo-deny or cargo-license) and parse the output, (3) at least one example of a denied or unknown license and the proposed remediation (replace, vendor, request exception), (4) the CI gate that prevents introducing a non-compliant license in a future PR.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "dep-analysis-supply-chain-multiprocess-copilot",
        title: "Dependency analysis: supply-chain risk on multi-process copilot-sdk",
        description: "Score third-party crate supply-chain risk and recommend mitigations through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::DependencyAnalysis,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Score supply-chain risk for top dependencies. Cover: (1) per-dep risk dimensions (maintainer count, release cadence, unsafe-block density, GitHub stars/issues, recent CVEs), (2) the threshold above which a dep requires vendoring or replacement, (3) at least three concrete deps from Cargo.toml scored against the rubric, (4) mitigations for the highest-risk dep (pin to specific commit, mirror to internal registry, fork). Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "dep-upgrade-major-bump-local",
        title: "Dependency upgrade: major-version bump on local harness",
        description: "Plan a breaking major-version dependency upgrade with a phased verification strategy on the single-process local harness.",
        class: BenchmarkClass::DependencyUpgrade,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Plan a major-version dependency upgrade. Pick one crate in Cargo.toml. Cover: (1) the breaking changes documented in the upstream CHANGELOG and which call sites in src/ are affected, (2) the migration order (update import sites first, then call sites, then idiomatic refactors) with a compile checkpoint after each, (3) the test surface that must pass and any new tests added for changed semantics, (4) the rollback plan if the upgrade reveals a runtime regression after merge.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "dep-upgrade-security-patch-multiprocess-terminal",
        title: "Dependency upgrade: emergency security patch on multi-process terminal-shell",
        description: "Apply an emergency CVE patch within hours through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::DependencyUpgrade,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Apply an emergency CVE patch. Cover: (1) the minimum diff (patch-version bump only, no transitive churn) and the cargo update --precise invocation, (2) the test selection executed under time pressure (security-relevant tests plus smoke suite) and the rationale for skipping the full matrix, (3) the deploy ordering (canary in production within an hour, full rollout within four), (4) the post-incident audit that confirms no regression escaped the abbreviated test run. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "documentation-readme-quickstart-local",
        title: "Documentation: README quickstart on local harness",
        description: "Author a runnable README quickstart for a Simard module on the single-process local harness.",
        class: BenchmarkClass::Documentation,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Write a quickstart section for the gym subsystem (src/gym/). Cover: (1) the minimum prerequisites (Rust toolchain, environment variables) and a three-line install command, (2) a runnable example that executes one BenchmarkScenario end-to-end and shows expected output, (3) where to find generated artifacts (run dir, report.json, report.txt), (4) links to the next-deeper docs (scenario reference, executor internals) so the reader has a clear progression.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "documentation-architecture-decision-multiprocess-rusty-clawd",
        title: "Documentation: architecture decision record on multi-process rusty-clawd",
        description: "Author an ADR for a recent architectural choice through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::Documentation,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Write an Architecture Decision Record. Cover: (1) the context and the forces driving the decision (constraints, prior art, current pain), (2) the decision in a single declarative sentence and the alternatives considered with their disqualifying properties, (3) the consequences (positive, negative, and neutral) with at least one observable metric, (4) the status (proposed/accepted/superseded) and the date. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "event-sourcing-snapshotting-local",
        title: "Event sourcing: aggregate snapshotting on local harness",
        description: "Design snapshotting to bound aggregate replay time on the single-process local harness.",
        class: BenchmarkClass::EventSourcing,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Design aggregate snapshotting. Cover: (1) the trigger policy (every N events, or on a time schedule) and the trade-off in storage vs replay latency, (2) the snapshot envelope (aggregate_id, version, payload, schema_version, taken_at) and the storage location, (3) the load procedure that combines the latest snapshot plus subsequent events to materialize current state, (4) the recovery path when a snapshot is corrupt or its schema_version is unsupported (fall back to full replay).",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "event-sourcing-poison-event-multiprocess-rusty-clawd",
        title: "Event sourcing: poison event handling on multi-process rusty-clawd",
        description: "Design quarantine and recovery for poison events that crash projection consumers through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::EventSourcing,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design poison-event handling. Cover: (1) detection (consumer panics or exceeds an error budget on a single event), (2) quarantine procedure that moves the offender to a dead-letter stream so the main projection can advance, (3) operator workflow to inspect, repair, or transform the event before replay, (4) the regression guard (schema validation at append time) that prevents a similar poison event in the future. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "feature-flag-targeting-rules-local",
        title: "Feature flagging: rule-based targeting on local harness",
        description: "Design rule-based flag targeting (by attribute, segment, or geography) on the single-process local harness.",
        class: BenchmarkClass::FeatureFlagging,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Design rule-based feature targeting. Cover: (1) the rule grammar (attribute equals/in/matches, AND/OR composition) and the maximum nesting allowed, (2) the precedence order between rules, default fallback, and explicit overrides, (3) deterministic evaluation given the same context so two services agree on the same flag value for the same user, (4) at least one concrete example: a flag that is on for region=EU AND plan in {pro, enterprise} with the resulting evaluation table.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "feature-flag-cleanup-multiprocess-copilot",
        title: "Feature flagging: stale-flag cleanup on multi-process copilot-sdk",
        description: "Plan retirement of long-lived feature flags whose rollout is complete through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::FeatureFlagging,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan stale-flag cleanup. Cover: (1) the criteria for declaring a flag stale (100 percent rollout for N weeks, or 0 percent and unowned), (2) the audit-and-confirm step with the owning team before removal, (3) the code-removal sequence (delete branch in code, then delete flag from store, never the reverse) and why, (4) the metric or report that tracks flag debt over time so it cannot silently re-accumulate. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "incident-response-comms-template-local",
        title: "Incident response: customer comms template on local harness",
        description: "Author a customer-facing status-page template for an active incident on the single-process local harness.",
        class: BenchmarkClass::IncidentResponse,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Author a status-page incident template. Cover: (1) the initial post-detection update (impact summary, affected feature, time of first user impact), (2) the cadence for follow-up updates and the trigger to escalate the visible severity, (3) the resolution post including the next-steps statement and a pointer to the eventual public post-mortem, (4) tone guidance that avoids speculation and prefers verifiable facts.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "incident-response-severity-matrix-multiprocess-rusty-clawd",
        title: "Incident response: severity matrix on multi-process rusty-clawd",
        description: "Define a severity classification matrix and the on-call response expectations through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::IncidentResponse,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Define an incident severity matrix. Cover: (1) the severity tiers (Sev1..Sev4) with measurable criteria (user impact, revenue impact, data integrity, regulatory exposure), (2) per-tier response expectations (page on-call, time to first acknowledgement, time to first comms), (3) the escalation rules between tiers and to leadership, (4) at least three worked examples that show how a real incident maps onto the matrix. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "i18n-date-number-formatting-local",
        title: "Internationalization review: date and number formatting on local harness",
        description: "Audit date, time, and number formatting for locale-correctness on the single-process local harness.",
        class: BenchmarkClass::InternationalizationReview,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Audit locale-aware formatting. Cover: (1) at least three call sites that hardcode a date, time, or number format (e.g., MM/DD/YYYY, comma decimal separator) and the locales they break, (2) the chosen formatting library or platform API and how it consumes the user's BCP-47 tag, (3) handling of timezones (store UTC, render in user's IANA zone) including DST transitions, (4) currency formatting including the position of the symbol and the locale-specific grouping separators.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "i18n-bidi-rendering-multiprocess-terminal",
        title: "Internationalization review: bidirectional text rendering on multi-process terminal-shell",
        description: "Audit bidirectional (RTL/LTR) text mixing in UI strings through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::InternationalizationReview,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit bidirectional text rendering. Cover: (1) at least one mixed RTL/LTR string in the UI (e.g., an Arabic name with an English product code) and the current rendering bug, (2) the use of Unicode bidi control characters (LRM, RLM, FSI/PDI) to enforce intended ordering, (3) layout mirroring for RTL locales (icons, progress bars, navigation chevrons), (4) test data set that covers Arabic, Hebrew, and a mixed-script case so future regressions are visible. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "migration-planning-data-backfill-local",
        title: "Migration planning: idempotent data backfill on local harness",
        description: "Plan a resumable, idempotent data backfill for a large table on the single-process local harness.",
        class: BenchmarkClass::MigrationPlanning,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Plan a resumable data backfill. Cover: (1) the batching strategy (bounded primary-key range per batch, target throughput, sleep between batches to protect production load), (2) the resume token persisted after each batch so a crash does not re-process completed rows, (3) idempotency of the per-row update so a re-run of an already-completed batch is a no-op, (4) verification (sampled checksum, row-count diff) that the backfill matches the intended end state.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "migration-planning-service-extraction-multiprocess-copilot",
        title: "Migration planning: extract a service from a monolith on multi-process copilot-sdk",
        description: "Plan extraction of a bounded context from a monolith into a separate service through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::MigrationPlanning,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan a service extraction. Cover: (1) the bounded context selected and the seam (interface, message contract) that becomes the network boundary, (2) the strangler-fig sequence (introduce facade, route a small percentage to the new service, expand) with concrete percentage milestones, (3) data ownership transfer (dual-write, read-from-new, drop-old) and the consistency window during each phase, (4) the rollback plan at every phase and the metric that proves the extracted service meets latency parity. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "observability-slo-definition-local",
        title: "Observability instrumentation: SLO definition on local harness",
        description: "Define an SLO and its supporting SLIs for a user-facing endpoint on the single-process local harness.",
        class: BenchmarkClass::ObservabilityInstrumentation,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Define an SLO and supporting SLIs. Cover: (1) the user journey the SLO protects and the SLI that proxies it (e.g., availability, latency p95, freshness), (2) the target (e.g., 99.9 percent over a 28-day rolling window) and the rationale tied to user expectations, (3) the error budget consumed per period and the policy when budget is exhausted, (4) the dashboard or alert query (PromQL or equivalent) that reports current consumption.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "observability-distributed-tracing-multiprocess-rusty-clawd",
        title: "Observability instrumentation: distributed tracing on multi-process rusty-clawd",
        description: "Plan rollout of distributed tracing across a request path that crosses processes through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::ObservabilityInstrumentation,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan distributed tracing rollout. Cover: (1) the propagation header standard (W3C traceparent) and how it is injected at every outbound boundary (HTTP, gRPC, message bus), (2) the sampling strategy (head-based vs tail-based) and the sample rate that balances signal vs cost, (3) the per-span attributes (operation name, status, attributes for tenant_id and request_id) that make the trace queryable, (4) the verification that a single request id is visible end-to-end across all hops. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "rate-limit-burst-shaping-local",
        title: "Rate limiting: burst shaping with leaky bucket on local harness",
        description: "Design a leaky-bucket smoother that converts bursty input into a steady downstream rate on the single-process local harness.",
        class: BenchmarkClass::RateLimiting,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Design a leaky-bucket smoother. Cover: (1) bucket capacity and constant drain rate parameters and how they relate to the downstream's stable RPS, (2) the queue-vs-reject behavior when the bucket is full (latency budget for queued requests vs immediate 429), (3) per-key vs global bucketing trade-off and the chosen strategy with rationale, (4) at least one concrete numeric example showing input burst of N RPS smoothed to drain rate R RPS over T seconds.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "rate-limit-tier-quotas-multiprocess-terminal",
        title: "Rate limiting: per-tier quotas on multi-process terminal-shell",
        description: "Design tier-based per-customer quotas with overage policy through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::RateLimiting,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Design per-tier customer quotas. Cover: (1) the tier table (Free/Pro/Enterprise) with concrete RPS and daily-call ceilings per tier, (2) overage policy (hard reject vs soft throttle vs metered billing) and how the choice is configured per tier, (3) the headers returned to clients (X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset) and how they remain accurate across instances, (4) the upgrade path that increases a customer's effective quota within seconds of a tier change. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "release-management-changelog-local",
        title: "Release management: changelog generation on local harness",
        description: "Generate a structured CHANGELOG entry from the merged commits since the last tag on the single-process local harness.",
        class: BenchmarkClass::ReleaseManagement,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Generate a CHANGELOG entry. Cover: (1) the source of truth (git log between previous tag and HEAD) and the filter that keeps merge commits or PR titles, (2) the categorisation (Added, Changed, Fixed, Security, Deprecated) and the heuristic mapping from commit-message prefixes (feat/fix/chore), (3) the breaking-change call-out format and where it appears, (4) the rendered preview for a sample range with at least three categorised entries.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "release-management-rollback-runbook-multiprocess-copilot",
        title: "Release management: rollback runbook on multi-process copilot-sdk",
        description: "Author the operator runbook to roll back a bad release within minutes through the multi-process topology with the copilot-sdk base type.",
        class: BenchmarkClass::ReleaseManagement,
        identity: "simard-gym",
        base_type: "copilot-sdk",
        topology: RuntimeTopology::MultiProcess,
        objective: "Author a rollback runbook. Cover: (1) the trigger conditions (which alert or signal authorises the rollback without further approval), (2) the exact commands or pipeline action that reverts to the previous artifact and the expected duration, (3) data-layer considerations (schema migrations that cannot be rolled back, feature flags that must be disabled in lockstep), (4) the verification step that confirms the previous version is once again serving traffic and the all-clear comms. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "safe-code-change-rename-private-fn-local",
        title: "Safe code change: rename a private function on local harness",
        description: "Plan a safe in-module rename of a private function on the single-process local harness.",
        class: BenchmarkClass::SafeCodeChange,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Plan a private-function rename. Pick a `fn` (non-pub) inside src/gym/ or src/runtime/. Cover: (1) the new name and why it improves clarity over the old, (2) every call site within the same module that needs updating and how the compiler will catch any miss, (3) confirmation that the symbol is not exported (no pub, no re-export) so the rename has no external impact, (4) the test that should still pass unchanged after the rename to prove behavior is preserved.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "safe-code-change-extract-helper-multiprocess-rusty-clawd",
        title: "Safe code change: extract helper function on multi-process rusty-clawd",
        description: "Plan extraction of a duplicated block into a private helper function through the multi-process topology with the rusty-clawd base type.",
        class: BenchmarkClass::SafeCodeChange,
        identity: "simard-gym",
        base_type: "rusty-clawd",
        topology: RuntimeTopology::MultiProcess,
        objective: "Plan an extract-helper refactor. Cover: (1) the duplicated block (cite at least two call sites in src/) and the proposed helper signature, (2) parameterisation choices (which captured locals become arguments vs remain ambient state) and the lifetime/borrow implications, (3) the visibility of the new helper (private to module, restricted with pub(crate)) and why, (4) the test surface that exercises the helper directly versus through the original call sites. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
    BenchmarkScenario {
        id: "session-quality-evidence-completeness-local",
        title: "Session quality: evidence completeness audit on local harness",
        description: "Audit the evidence captured during a bounded engineering session for completeness on the single-process local harness.",
        class: BenchmarkClass::SessionQuality,
        identity: "simard-gym",
        base_type: "local-harness",
        topology: RuntimeTopology::SingleProcess,
        objective: "Audit a bounded session for evidence completeness. Cover: (1) the inputs that were captured (objective, base type, identity) and any inputs that should have been captured but were not, (2) the runtime evidence records emitted and whether they cover plan, execution, and reflection phases, (3) the artifacts produced (report.json, report.txt, review.json) and whether each is well-formed and complete, (4) at least one concrete gap with the proposed instrumentation change to close it.",
        expected_min_runtime_evidence: 3,
    },
    BenchmarkScenario {
        id: "session-quality-handoff-fidelity-multiprocess-terminal",
        title: "Session quality: handoff fidelity on multi-process terminal-shell",
        description: "Audit the fidelity of a session handoff (export then restore) through the multi-process topology with the terminal-shell base type.",
        class: BenchmarkClass::SessionQuality,
        identity: "simard-gym",
        base_type: "terminal-shell",
        topology: RuntimeTopology::MultiProcess,
        objective: "Audit a session handoff for fidelity. Cover: (1) the exported state fields (memory record count, evidence record count, session phase, objective) compared against the live session at export time, (2) the restored fields after import and any mismatch, (3) the contract that handoff must preserve (no silent dropping of records, no reordering of evidence) and how to verify it, (4) one concrete failure mode (e.g., truncated memory list under a size limit) and the proposed instrumentation to detect it. Confirm the multi-process transport appears in runtime evidence.",
        expected_min_runtime_evidence: 4,
    },
];
