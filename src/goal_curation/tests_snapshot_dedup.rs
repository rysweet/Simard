...
fn live_snapshot_count(memory: &dyn CognitiveMemoryOps) -> usize {
    memory
...
fn repeated_snapshot_saves_supersede_not_duplicate() {
    ...
    save_goal_board(&board, memory.ops()).expect("save v1");
    assert_eq!(
        live_snapshot_count(memory.ops()),
        1,
    ...
    save_goal_board(&board, memory.ops()).expect("save v2");
    assert_eq!(live_snapshot_count(memory.ops()), 1, "v2 supersedes v1");

    // Save #3 (changed): supersedes v2 — still one live snapshot.
    add_active_goal(&mut board, active_goal("goal-gamma", 3)).unwrap();
    save_goal_board(&board, memory.ops()).expect("save v3");
    assert_eq!(
        live_snapshot_count(memory.ops()),
        1,
    ...
    let loaded = load_goal_board(memory.ops()).expect("load board");
