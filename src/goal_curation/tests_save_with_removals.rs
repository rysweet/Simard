...
/// opened adapter so each step exercises the same persistence layer as
/// production callers.
fn persist(board: &GoalBoard, root: &std::path::Path) {
    let adapter = launch_writer_adapter(root).expect("writer adapter");
    save_goal_board(board, adapter.ops()).expect("save_goal_board");
}
...
fn empty_removals_is_equivalent_to_save_goal_board() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    save_goal_board_with_removals(&board, &[], adapter.ops())
...
fn removes_active_goal_present_on_in_flight_board() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    save_goal_board_with_removals(&board, &["doomed".to_string()], adapter.ops())
...
fn removes_backlog_item_in_addition_to_active() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    save_goal_board_with_removals(&board, &removals, adapter.ops())
        .expect("save_goal_board_with_removals");
...
fn removal_defeats_pr_1926_resurrection_from_persisted_snapshot() {
    ...
    add_active_goal(&mut in_flight, active_goal("keeper", 1)).unwrap();
    {
        ...
        save_goal_board(&in_flight, adapter.ops()).expect("plain save");
    }
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    save_goal_board_with_removals(&in_flight, &["doomed".to_string()], adapter.ops())
        .expect("save_goal_board_with_removals");
...
fn unknown_removal_ids_are_silent_no_ops() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    let result = save_goal_board_with_removals(&board, &removals, adapter.ops());
    assert!(
        result.is_ok(),
...
fn empty_board_with_removals_is_idempotent() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    let r1 = save_goal_board_with_removals(&board, &removals, adapter.ops());
    let r2 = save_goal_board_with_removals(&board, &removals, adapter.ops());
...
fn concurrent_unrelated_persisted_goal_survives_removal_of_other_id() {
    ...
    let adapter = launch_writer_adapter(&root).expect("writer adapter");
    save_goal_board_with_removals(&operator_in_flight, &["doomed".to_string()], adapter.ops())
        .expect("save_goal_board_with_removals");
