...
//!   3. `memory_ipc::launcher::launch_writer_adapter`
//!
//! Each site asserts:
...
//!    `store_fact`, and `launch_writer_adapter`.
//!
...
use crate::memory_ipc::{TEST_ALLOW_LIVE_STATE_ENV, launch_writer_adapter};
...
fn save_goal_board_against_tempdir_state_root_does_not_trip_guard() {
    ...
    let adapter =
        launch_writer_adapter(tmp.path()).expect("writer adapter against hermetic state root");
...
fn save_goal_board_with_removals_against_tempdir_state_root_does_not_trip_guard() {
    ...
    let board = sample_board();
    let adapter = launch_writer_adapter(tmp.path()).expect("writer adapter");
...
fn save_goal_board_against_home_simard_state_root_trips_guard() {
    ...
    let panicked = catch_unwind(AssertUnwindSafe(|| {
        // Open an adapter by-path so we can exercise the save site
        ...
        let adapter = crate::memory_ipc::WriterAdapter::from_ops_for_test(Box::new(mem));
        let _ = save_goal_board(&board, adapter.ops());
    }));
    assert!(
        panicked.is_err(),
...
#[test]
#[serial(cognitive_memory)]
fn launch_writer_adapter_against_home_simard_state_root_trips_guard() {
    let (_home_tmp, home, state_root) = fake_home_under_temp();
    let _home_guard = EnvGuard::set("HOME", home.to_str().unwrap());
    let _state_unset = EnvGuard::unset(STATE_ROOT_ENV);
    let _allow = EnvGuard::unset(TEST_ALLOW_LIVE_STATE_ENV);
    ...
    let panicked = catch_unwind(AssertUnwindSafe(|| {
        let _ = launch_writer_adapter(&state_root);
    }));
    assert!(
        panicked.is_err(),
        "launch_writer_adapter MUST panic when state_root is under \
