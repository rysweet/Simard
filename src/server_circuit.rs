...
use crate::server_transport::{ServerRequest, ServerResponse, ServerTransport};
...
/// An adapter transport wrapper that implements the circuit breaker pattern.
///
...
pub struct CircuitBreakerTransport<T: ServerTransport> {
    inner: T,
...
impl<T: ServerTransport> CircuitBreakerTransport<T> {
...
impl<T: ServerTransport> ServerTransport for CircuitBreakerTransport<T> {
    fn call(&self, request: ServerRequest) -> SimardResult<ServerResponse> {
        {
            let mut state = self
                ...
                .map_err(|_| SimardError::StoragePoisoned {
                    store: "adapter-circuit".to_string(),
                })?;
            if !self.should_allow_call(&mut state) {
                return Err(SimardError::ServerCircuitOpen {
                    adapter: self.inner.descriptor().identity.clone(),
                });
            }
        }
        ...
        match self.inner.call(request) {
            Ok(response) => {
                let is_transport_error = response
                    .error
                    .as_ref()
                    .is_some_and(|e| e.code == crate::server_transport::SERVER_ERROR_TRANSPORT);
                let mut state = self
                    .state
                    .lock()
                    .map_err(|_| SimardError::StoragePoisoned {
...
mod tests {
    ...
    use crate::server_transport::{
        SERVER_ERROR_INTERNAL, SERVER_ERROR_TRANSPORT, ServerErrorPayload, ServerHealth,
    ...
    fn closed_circuit_passes_calls_through() {
        let inner = InMemoryServerTransport::echo("test");
    ...
    fn circuit_opens_after_threshold_failures() {
        ...
        let inner = InMemoryServerTransport::new("fail", move |_method, _params| {
            counter.fetch_add(1, Ordering::SeqCst);
            Err(ServerErrorPayload {
                code: SERVER_ERROR_TRANSPORT,
    ...
    fn application_errors_do_not_trip_circuit() {
        let inner = InMemoryServerTransport::new("app-err", |_method, _params| {
            Err(ServerErrorPayload {
    ...
    fn circuit_transitions_to_half_open_after_cooldown() {
        let inner = InMemoryServerTransport::new("fail-then-ok", |_method, _params| {
            Err(ServerErrorPayload {
                code: SERVER_ERROR_TRANSPORT,
    ...
    fn successful_probe_closes_circuit() {
        let call_count = std::sync::Arc::new(AtomicU32::new(0));
        let counter = call_count.clone();
        let inner = InMemoryServerTransport::new("recover", move |_method, params| {
