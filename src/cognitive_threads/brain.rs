//! The [`Brain`] — the cognitive-thread scheduler (Appendix A.5).
//!
...
//! graceful shutdown live in [`Brain::run_due`]; the registry/bookkeeping
...
// FROZEN WIRE VALUE: operator-set environment variable literal.
...
/// The Brain — Simard's whole cognition: the executive that schedules the
/// cognitive threads (the OODA thread with its orient/decide/act reasoners,
/// plus maintenance, engineer-log-analysis, and future background threads)
/// over the shared cognitive memory.
pub struct Brain {
...
impl Brain {
    /// Build a `Brain` with the budget read from [`BRAIN_NONCRITICAL_BUDGET_ENV`]
    /// (default [`DEFAULT_BUDGET`]).
    pub fn new() -> Self {
        let max = std::env::var(BRAIN_NONCRITICAL_BUDGET_ENV)
            .ok()
    ...
    /// Build a `Brain` with an explicit non-critical per-tick budget (test seam
    /// — avoids env mutation).
