...
use crate::terminal_engineer::{
    ...
    TERMINAL_MODE_BOUNDARY, TerminalEngineerContext, load_runtime_handoff_snapshot,
};
...
impl EngineerReadView {
    pub(super) fn load(state_root: PathBuf) -> crate::SimardResult<Self> {
        ...
        Ok(Self {
            ...
            .to_string(),
            terminal_engineer_context: TerminalEngineerContext::from_engineer_evidence(
                &handoff.evidence_records,
            )?,
    ...
    fn render_lines(&self) -> Vec<String> {
        ...
        match &self.terminal_engineer_context {
            Some(context) => {
...
mod tests {
    ...
    // --- render_lines: terminal adapter context branches ---

    fn make_view_with_adapter(
        adapter: Option<TerminalEngineerContext>,
        goals: Vec<String>,
        ...
        EngineerReadView {
            ...
            verification_status: "passed".to_string(),
            verification_summary: "all checks passed".to_string(),
            terminal_engineer_context: adapter,
    ...
    #[test]
    fn render_lines_terminal_engineer_some_with_last_output() {
        let adapter = TerminalEngineerContext {
        ...
        let view = make_view_with_adapter(Some(adapter), vec![], vec![], 0, 0);
    ...
    #[test]
    fn render_lines_terminal_engineer_some_without_last_output() {
        let adapter = TerminalEngineerContext {
            continuity_source: "state-root".to_string(),
            handoff_file_name: "terminal-handoff.json".to_string(),
            working_directory: "/home/user/project".to_string(),
    ...
    #[test]
    fn render_lines_terminal_engineer_none_shows_not_available() {
        let view = make_view_with_adapter(None, vec![], vec![], 0, 0);
        let output = view.render_lines().join("\n");
        ...
        assert!(
            ...
            "should not show handoff details when adapter is None"
        );
    }
    ...
    fn render_lines_multiple_active_goals() {
        ...
        let view = make_view_with_adapter(None, goals, vec![], 0, 0);
        let output = view.render_lines().join("\n");
    ...
    fn render_lines_multiple_carried_decisions() {
        ...
        let view = make_view_with_adapter(None, vec![], decisions, 0, 0);
        let output = view.render_lines().join("\n");
    ...
    fn render_lines_shows_memory_and_evidence_counts() {
        let view = make_view_with_adapter(None, vec![], vec![], 7, 14);
        let output = view.render_lines().join("\n");
    ...
    // --- render_lines: section 6 mode boundary appears for adapter context ---

    #[test]
    fn render_lines_adapter_context_emits_terminal_mode_boundary() {
        let adapter = TerminalEngineerContext {
        ...
        assert_eq!(
            ...
            "terminal mode boundary should appear exactly once in adapter context section"
        );
