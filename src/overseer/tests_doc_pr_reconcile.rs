...
use crate::overseer::config::DEFAULT_OVERSEER_AUTHOR_LOGIN;
use crate::overseer::doc_pr_reconcile::{
    AUTO_DOC_PR_LABEL, AUTO_DOC_PR_TITLE_MARKER, CloseReason, is_auto_doc_pr, reconcile_doc_prs,
    run_doc_pr_reconcile_with_author,
};
...
/// The auto-doc author identity used by the fixtures — Simard's ENGINEER / OODA
/// gh login (what `config::automerge_author` resolves in production). It is
/// deliberately DISTINCT from [`DEFAULT_OVERSEER_AUTHOR_LOGIN`] so a fixture can
/// never accidentally bind the gate's expected author to the overseer-bot login
/// (the masked-bug the HIGH review finding exposed).
const ENGINEER_AUTHOR: &str = "simard-engineer";
...
fn a_fully_qualified_pr_is_an_auto_doc_pr() {
    assert!(
        is_auto_doc_pr(&auto_doc_pr(42, "MERGEABLE"), ENGINEER_AUTHOR),
...
fn empty_author_fails_closed_as_human() {
    ...
    assert!(
        !is_auto_doc_pr(&pr, ENGINEER_AUTHOR),
...
fn a_human_authored_pr_with_the_same_title_is_not_auto_doc() {
    assert!(
        !is_auto_doc_pr(&human_pr(42), ENGINEER_AUTHOR),
...
fn a_pr_missing_the_label_is_not_auto_doc() {
    ...
    assert!(
        !is_auto_doc_pr(&pr, ENGINEER_AUTHOR),
        "the auto-doc label must be present"
    );
}
...
#[test]
fn an_overseer_bot_authored_pr_is_not_an_auto_doc_candidate() {
    // Regression for the HIGH review finding: the gate USED to hard-code the
    // expected author to `simard-overseer[bot]` (DEFAULT_OVERSEER_AUTHOR_LOGIN),
    // but real auto-doc PRs are authored under Simard's ENGINEER identity. An
    // auto-doc-SHAPED PR carrying the overseer-bot login must NOT match the
    // engineer expected-author — otherwise the gate would key on the wrong
    // identity and match zero real PRs in production (inert).
    let mut bot = auto_doc_pr(42, "MERGEABLE");
    bot.author = DEFAULT_OVERSEER_AUTHOR_LOGIN.to_string();
    assert!(
        !is_auto_doc_pr(&bot, ENGINEER_AUTHOR),
        "an overseer-bot-authored PR must NOT match the engineer auto-doc identity"
    );
    // And the engineer-authored auto-doc PR IS a candidate — the gate is now
    // keyed on the identity that actually opens auto-doc PRs.
    assert!(
        is_auto_doc_pr(&auto_doc_pr(42, "MERGEABLE"), ENGINEER_AUTHOR),
        "the engineer-authored auto-doc PR is a candidate under the engineer identity"
    );
    // Symmetry check: if the deployment's auto-doc author WERE the bot, the bot
    // PR matches under THAT expected author — proving the gate tracks whatever
    // identity is resolved at the I/O boundary rather than a frozen constant.
    assert!(
        is_auto_doc_pr(&bot, DEFAULT_OVERSEER_AUTHOR_LOGIN),
        "the gate matches whichever author identity is injected"
    );
}

#[test]
fn an_empty_expected_author_fails_closed() {
    // `config::automerge_author()` returns None (=> empty string here) when
    // SIMARD_AUTOMERGE_AUTHOR is unset. An unresolved identity must match NOTHING
    // so the reconciler stays fail-closed rather than closing PRs under a blank
    // author (which an empty pr.author would otherwise satisfy).
    assert!(
        !is_auto_doc_pr(&auto_doc_pr(42, "MERGEABLE"), ""),
        "an empty expected author (unset SIMARD_AUTOMERGE_AUTHOR) must match nothing"
    );
    let mut blank_authored = auto_doc_pr(42, "MERGEABLE");
    blank_authored.author = String::new();
    assert!(
        !is_auto_doc_pr(&blank_authored, ""),
        "empty-vs-empty must NOT match — both author guards fail closed"
    );
}
...
fn zero_candidates_is_a_no_op() {
    let decision = reconcile_doc_prs(&[], ENGINEER_AUTHOR);
...
fn a_single_candidate_is_canonical_with_no_closes() {
    let decision = reconcile_doc_prs(&[auto_doc_pr(7, "MERGEABLE")], ENGINEER_AUTHOR);
    assert_eq!(
        decision.canonical,
        Some(7),
...
fn non_auto_doc_prs_are_ignored_entirely() {
    ...
    let decision = reconcile_doc_prs(
        &[human_pr(99), auto_doc_pr(7, "MERGEABLE")],
        ENGINEER_AUTHOR,
    );
    assert_eq!(
        decision.canonical,
...
fn executor_closes_the_superseded_prs_by_number() {
    ...
    let report =
        run_doc_pr_reconcile_with_author(REPO, &gh, ENGINEER_AUTHOR).expect("reconcile succeeds");
...
fn executor_is_fail_closed_on_a_list_error() {
    ...
    let result = run_doc_pr_reconcile_with_author(REPO, &gh, ENGINEER_AUTHOR);
    assert!(
        result.is_err(),
...
#[test]
fn executor_is_inert_when_the_author_identity_mismatches() {
    // The exact production failure the HIGH finding described: title-marker,
    // draft, labelled auto-doc PRs are present, but the resolved expected author
    // does NOT match the PRs' author. The gate matches ZERO candidates, so the
    // executor closes NOTHING (fail-closed) rather than acting on a wrong or
    // mis-resolved identity. (The executor also emits a LOUD warn here.)
    let gh = FakeDocPrClient::with(vec![
        auto_doc_pr(10, "MERGEABLE"),
        auto_doc_pr(20, "MERGEABLE"),
    ]);
    let report = run_doc_pr_reconcile_with_author(REPO, &gh, DEFAULT_OVERSEER_AUTHOR_LOGIN)
        .expect("reconcile succeeds (inert)");
    assert_eq!(
        report.canonical, None,
        "no candidate matches the mismatched author => no canonical"
    );
    assert!(
        report.closed.is_empty() && gh.closed_numbers().is_empty(),
        "an author mismatch must close NOTHING (fail-closed inert pass)"
    );
    assert_eq!(
        report.skipped, 2,
        "both title-marker PRs are counted as skipped non-candidates"
    );
}

#[test]
fn executor_is_inert_when_the_author_is_unresolved() {
    // SIMARD_AUTOMERGE_AUTHOR unset => automerge_author() is None => empty
    // expected author. The pass must be fully inert (and warn LOUD).
    let gh = FakeDocPrClient::with(vec![
        auto_doc_pr(10, "MERGEABLE"),
        auto_doc_pr(20, "MERGEABLE"),
    ]);
    let report =
        run_doc_pr_reconcile_with_author(REPO, &gh, "").expect("reconcile succeeds (inert)");
    assert_eq!(report.canonical, None);
    assert!(
        gh.closed_numbers().is_empty(),
        "an unresolved author identity must close NOTHING"
    );
}
