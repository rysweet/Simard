...
/// build proceeds without the rail rather than panicking. The roster is mutable,
/// deploy-durable identity-scoped state under
/// `<state_root>/identity/<identity>/curated/stewarded_repos.toml`, seeded once
/// from the identity default and curated agentically (`simard roster add|remove`).
fn build_ecosystem_observer(
    ...
    use crate::overseer::ecosystem_observe::{
        RecipeEcosystemObserver, SpawnEcosystemRecipeRunner, resolve_daemon_stewarded_roster,
    };

    let roster = match resolve_daemon_stewarded_roster(state_root) {
        Ok(roster) => roster,
        Err(error) => {
            tracing::warn!(
                ...
                "[simard] ecosystem-observe NOT wired: failed to resolve the identity-curated stewarded roster",
            );
            return None;
        }
    };
...
/// while the merge ACTION stays behind the objective + agentic gate) — keeping
/// exactly one roster source of truth.
fn build_merge_queue_reasoner(
    ...
    use crate::overseer::ecosystem_observe::resolve_daemon_stewarded_roster;
    ...
    let roster = match resolve_daemon_stewarded_roster(state_root) {
        ...
        Err(error) => {
            tracing::warn!(
                ...
                error = %error,
                state_root = %state_root.display(),
                "[simard] observe-merge-queue NOT wired: failed to resolve the identity-curated governed roster",
            );
            return None;
        }
    };
