...
pub struct Overseer {
    ...
    /// ecosystem-observe recipe. Resolved once from the ACTIVE identity's
    /// mutable, curated roster (identity-scoped state, seeded from the identity
    /// default) by `build_overseer`; empty (and the pass skipped) until wired.
    ...
    /// `observe-merge-queue` recipe as the DEFAULT scope. Resolved once from the
    /// ACTIVE identity's mutable, curated roster (identity-scoped state, seeded
    /// from the identity default) by `build_overseer`; the resolved scope
