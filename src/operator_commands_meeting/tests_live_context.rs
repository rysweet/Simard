...
#[serial_test::serial(cognitive_memory)]
#[test]
fn empty_memory_uses_env_var_operator_name() {
    let _lock = ENV_MUTEX.lock().unwrap();
...
#[test]
fn includes_memory_meeting_facts() {
    let memory = memory_with_meeting_facts();
    let ctx = build_live_meeting_context(&memory).unwrap();
    ...
    assert!(
        ...
        "expected meeting content from memory"
    );
}

#[test]
fn includes_decision_facts() {
    let memory = memory_with_specific_facts("decision:", "decision", "Use Rust for backend");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn includes_goal_facts() {
    let memory = memory_with_specific_facts("goal:", "goal", "Complete API refactor");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn includes_operator_facts() {
    let memory = memory_with_specific_facts("operator:", "operator", "Custom operator identity");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
        ctx.contains("Operator Context"),
    ...
    assert!(
        ...
        "expected operator content from memory"
    );
    // Should NOT contain any default operator text when memory provides facts
    assert!(
        !ctx.contains("Ryan Sweet"),
        "should not contain default operator when memory provides custom operator"
    );
}

#[test]
fn includes_project_facts() {
    let memory = memory_with_specific_facts("project:", "project", "CustomProject — custom suite");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
        ctx.contains("Known Projects"),
    ...
    assert!(
        ...
        "expected project content from memory"
    );
}

#[test]
fn includes_research_facts() {
    let memory = memory_with_specific_facts("research:", "research", "Investigating LLM patterns");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn includes_improvement_facts() {
    let memory =
        memory_with_specific_facts("improvement:", "improvement", "Add better error handling");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn with_all_fact_types() {
    let memory = memory_with_all_fact_types();
    let ctx = build_live_meeting_context(&memory).unwrap();
...
fn no_defaults_when_operator_present() {
    let memory = memory_with_specific_facts("operator:", "operator", "Custom operator");
    let ctx = build_live_meeting_context(&memory).unwrap();
...
fn no_defaults_when_project_present() {
    let memory = memory_with_specific_facts("project:", "proj", "My Custom Project");
    let ctx = build_live_meeting_context(&memory).unwrap();
...
// ── empty_memory helper validation ─────────────────────────────────

#[test]
fn empty_memory_returns_empty_search_results() {
    let memory = empty_memory();
    let facts = memory
...
#[test]
fn empty_memory_has_operator_section_only() {
    let memory = empty_memory();
    let ctx = build_live_meeting_context(&memory).unwrap();
    // With empty memory, only the operator default section appears (no hardcoded projects)
    assert!(ctx.contains("## Operator Context"));
    assert!(!ctx.contains("## Known Projects"));
...
#[test]
fn empty_memory_omits_hardcoded_projects() {
    let memory = empty_memory();
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn research_section_is_bulleted() {
    let memory = memory_with_specific_facts("research:", "research", "LLM alignment research");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn improvement_section_is_bulleted() {
    let memory = memory_with_specific_facts("improvement:", "improvement", "Better error recovery");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn operator_section_is_bulleted() {
    let memory = memory_with_specific_facts("operator:", "operator", "Custom operator context");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
...
fn project_section_is_bulleted() {
    let memory = memory_with_specific_facts("project:", "project", "CustomProject — testing");
    let ctx = build_live_meeting_context(&memory).unwrap();
    assert!(
        ctx.contains("- CustomProject"),
...
// ── empty_memory: additional validation ─────────────────────────────

#[test]
fn empty_memory_search_returns_empty_for_various_prefixes() {
    let memory = empty_memory();
    for prefix in &[
        "meeting:",
        "decision:",
        ...
        let facts = memory.search_facts(prefix, 10, 0.0).unwrap_or_default();
