...
#[test]
fn defaults_with_empty_adapter() {
    ...
    let ctx = build_live_meeting_context(&adapter).unwrap();
...
#[serial_test::serial(cognitive_memory)]
#[test]
fn empty_adapter_uses_env_var_operator_name() {
    let _lock = ENV_MUTEX.lock().unwrap();
...
#[test]
fn includes_adapter_meeting_facts() {
    let adapter = adapter_with_meeting_facts();
    let ctx = build_live_meeting_context(&adapter).unwrap();
    ...
    assert!(
        ...
        "expected meeting content from adapter"
    );
}

#[test]
fn includes_decision_facts() {
    let adapter = adapter_with_specific_facts("decision:", "decision", "Use Rust for backend");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn includes_goal_facts() {
    let adapter = adapter_with_specific_facts("goal:", "goal", "Complete API refactor");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn includes_operator_facts() {
    let adapter = adapter_with_specific_facts("operator:", "operator", "Custom operator identity");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
        ctx.contains("Operator Context"),
    ...
    assert!(
        ...
        "expected operator content from adapter"
    );
    // Should NOT contain any default operator text when adapter provides facts
    assert!(
        !ctx.contains("Ryan Sweet"),
        "should not contain default operator when adapter provides custom operator"
    );
}
...
fn includes_project_facts() {
    let adapter =
        adapter_with_specific_facts("project:", "project", "CustomProject — custom suite");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
        ctx.contains("Known Projects"),
    ...
    assert!(
        ...
        "expected project content from adapter"
    );
}
...
fn includes_research_facts() {
    let adapter =
        adapter_with_specific_facts("research:", "research", "Investigating LLM patterns");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn includes_improvement_facts() {
    let adapter =
        adapter_with_specific_facts("improvement:", "improvement", "Add better error handling");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn with_all_fact_types() {
    let adapter = adapter_with_all_fact_types();
    let ctx = build_live_meeting_context(&adapter).unwrap();
...
fn no_defaults_when_operator_present() {
    let adapter = adapter_with_specific_facts("operator:", "operator", "Custom operator");
    let ctx = build_live_meeting_context(&adapter).unwrap();
...
fn no_defaults_when_project_present() {
    let adapter = adapter_with_specific_facts("project:", "proj", "My Custom Project");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    // When project facts present, should use adapter data not defaults
...
// ── empty_adapter helper validation ─────────────────────────────────

#[test]
fn empty_adapter_returns_empty_search_results() {
    let adapter = empty_adapter();
    let facts = adapter
...
#[test]
fn empty_adapter_has_operator_section_only() {
    let adapter = empty_adapter();
    let ctx = build_live_meeting_context(&adapter).unwrap();
    // With empty adapter, only the operator default section appears (no hardcoded projects)
    assert!(ctx.contains("## Operator Context"));
    assert!(!ctx.contains("## Known Projects"));
...
#[test]
fn empty_adapter_omits_hardcoded_projects() {
    let adapter = empty_adapter();
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn research_section_is_bulleted() {
    let adapter = adapter_with_specific_facts("research:", "research", "LLM alignment research");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn improvement_section_is_bulleted() {
    let adapter =
        adapter_with_specific_facts("improvement:", "improvement", "Better error recovery");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn operator_section_is_bulleted() {
    let adapter = adapter_with_specific_facts("operator:", "operator", "Custom operator context");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
...
fn project_section_is_bulleted() {
    let adapter = adapter_with_specific_facts("project:", "project", "CustomProject — testing");
    let ctx = build_live_meeting_context(&adapter).unwrap();
    assert!(
        ctx.contains("- CustomProject"),
...
// ── empty_adapter: additional validation ─────────────────────────────

#[test]
fn empty_adapter_search_returns_empty_for_various_prefixes() {
    let adapter = empty_adapter();
    for prefix in &[
        "meeting:",
        "decision:",
        ...
        let facts = adapter.search_facts(prefix, 10, 0.0).unwrap_or_default();
