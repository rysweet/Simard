...
pub fn run_goal_curation_read_probe(
    ...
    let adapter = crate::memory_ipc::launch_writer_adapter(&state_root)?;
    let board = crate::goal_curation::load_goal_board(adapter.ops())?;
...
mod tests {
    ...
    fn goal_curation_read_probe_with_missing_directory() {
        ...
        // memory adapter handles an empty board gracefully, so this should
    ...
    fn goal_curation_read_probe_with_seeded_cognitive_memory() {
        ...
        // TempDir-rooted state and the adapter socket follows that root
        // (issues #1923 / #1925).
        ...
        let adapter =
            crate::memory_ipc::launch_writer_adapter(state.state_root()).expect("writer adapter");
    ...
    fn banner_and_goal_curation_read_agree_on_shared_store_with_seeded_goals() {
        ...
        let adapter =
            crate::memory_ipc::launch_writer_adapter(&state_root).expect("writer adapter");
        let board = seeded_board(4);
        crate::goal_curation::save_goal_board(&board, adapter.ops()).expect("seed board");

        // Path A: greeting banner reads via the same adapter.
        let banner_lines = build_greeting_banner(Some(adapter.ops()));
        ...
        let board_via_read = crate::goal_curation::load_goal_board(adapter.ops())
    ...
    /// against the same adapter both code paths would use.
    #[test]
    #[serial_test::serial(cognitive_memory)]
    fn pillar11_banner_and_read_agree_with_seeded_goals_fast() {
        ...
        // The invariant is that reading the same adapter yields the same count.
        let board_a =
            crate::goal_curation::load_goal_board(adapter.ops()).expect("load board (banner path)");
        let board_b =
            crate::goal_curation::load_goal_board(adapter.ops()).expect("load board (read path)");
    ...
    fn banner_and_goal_curation_read_agree_on_shared_store_when_empty() {
        ...
        let board_via_read =
            crate::goal_curation::load_goal_board(adapter.ops()).expect("read empty board");
        assert_eq!(
    ...
    fn pillar11_banner_and_read_agree_when_empty_fast() {
        ...
        let board = crate::goal_curation::load_goal_board(adapter.ops()).expect("load empty board");
    ...
    fn goal_curation_read_default_state_root_resolves_to_canonical_daemon_store() {
        ...
        // Both the greeting banner (via meeting_session::launch_real_meeting_adapter)
