...
pub(crate) fn compute_diff_for_review(
    ...
    // Clear git env vars that may be inherited from a parent worktree context
    // (e.g. GIT_DIR pointing to the outer repo when running inside cargo test).
    for key in super::CLEARED_GIT_ENV_VARS {
        cmd.env_remove(key);
    }
    match cmd.output() {
