...
use crate::server_transport::{
    SERVER_ERROR_TIMEOUT, SERVER_ERROR_TRANSPORT, ServerErrorPayload, ServerRequest, ServerResponse,
};

use crate::server_transport::ServerTransport;

/// An adapter transport that spawns a Python subprocess and communicates via
...
pub struct SubprocessServerTransport {
    adapter_name: String,
...
impl SubprocessServerTransport {
    /// Create a new subprocess adapter transport.
    ///
    /// - `adapter_name`: human-readable name for error reporting
    /// - `python_script`: path to the Python adapter server script
    ...
    fn spawn_child(&self) -> SimardResult<ManagedChild> {
        ...
        let mut child = command
            .spawn()
            .map_err(|error| SimardError::ServerSpawnFailed {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| SimardError::ServerSpawnFailed {
                adapter: self.adapter_name.clone(),
    ...
    fn ensure_child<'a>(
        ...
        state
            .child
            .as_mut()
            .ok_or_else(|| SimardError::ServerTransportError {
                adapter: "subprocess".into(),
                reason: "child process unexpectedly missing after spawn".into(),
            })
    }

    fn send_request(child: &mut ManagedChild, request: &ServerRequest) -> SimardResult<()> {
        let mut line =
            serde_json::to_string(request).map_err(|error| SimardError::ServerProtocolError {
                adapter: "subprocess".to_string(),
        ...
        child
            .stdin
            .flush()
            .map_err(|error| SimardError::ServerTransportError {
                adapter: "subprocess".to_string(),
            ...
            if remaining.is_zero() {
                return Ok(ServerResponse {
                    ...
                    error: Some(ServerErrorPayload {
                        code: SERVER_ERROR_TIMEOUT,
                        message: format!("adapter '{adapter_name}' timed out after {timeout:?}"),
                    }),
                });
            }
                ...
                Ok(_) => {
                    ...
                    let response: ServerResponse =
                        serde_json::from_str(trimmed).map_err(|error| {
                            SimardError::ServerProtocolError {
                                adapter: adapter_name.to_string(),
...
impl ServerTransport for SubprocessServerTransport {
    fn call(&self, request: ServerRequest) -> SimardResult<ServerResponse> {
        let mut state = self
            .state
            .lock()
            .map_err(|_| SimardError::StoragePoisoned {
                store: format!("adapter:{}", self.adapter_name),
            })?;
        let expected_id = request.id.clone();
        let child = self.ensure_child(&mut state)?;
        Self::send_request(child, &request)?;
        let response = Self::read_response(child, &expected_id, self.timeout, &self.adapter_name)?;
        if let Some(ref error) = response.error
            && error.code == SERVER_ERROR_TRANSPORT
        {
            state.child = None;
        }
        Ok(response)
    ...
    fn descriptor(&self) -> BackendDescriptor {
        BackendDescriptor::for_runtime_type::<Self>(
            ...
            format!("server_transport::subprocess::{}", self.adapter_name),
...
impl Drop for SubprocessServerTransport {
    fn drop(&mut self) {
...
mod tests {
    use super::*;
    use crate::server_transport::SERVER_ERROR_TIMEOUT;
    ...
    fn subprocess_transport_descriptor_includes_script_path() {
        ...
        assert!(desc.identity.contains("test_adapter.py"));
    }
    ...
    fn read_response_returns_timeout_error_when_subprocess_is_silent() {
        ...
        // read_response should return a ServerResponse with SERVER_ERROR_TIMEOUT.
        ...
        let result = SubprocessServerTransport::read_response(
            &mut managed,
            "test-id-1",
        ...
        assert_eq!(
            error.code, SERVER_ERROR_TIMEOUT,
            "error code should be SERVER_ERROR_TIMEOUT"
        );
        assert!(
