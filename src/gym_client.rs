...
const CLIENT_NAME: &str = "simard-gym-eval";
...
impl GymClient {
    ...
    pub fn list_scenarios(&self) -> SimardResult<Vec<GymScenario>> {
        ...
        unpack_rpc_response(CLIENT_NAME, "gym.list_scenarios", response)
    }
    ...
    pub fn run_scenario(&self, scenario_id: &str) -> SimardResult<GymScenarioResult> {
        ...
        unpack_rpc_response(CLIENT_NAME, "gym.run_scenario", response)
    }
    ...
    pub fn run_suite(&self, suite_id: &str) -> SimardResult<GymSuiteResult> {
        ...
        unpack_rpc_response(CLIENT_NAME, "gym.run_suite", response)
    }
}
...
mod tests {
    ...
    fn fixed_result_transport(result: serde_json::Value) -> InMemoryRpcTransport {
        InMemoryRpcTransport::new("gym-test", move |_method, _params| Ok(result.clone()))
    }

    fn fixed_error_transport(code: i32, message: &str) -> InMemoryRpcTransport {
        let msg = message.to_string();
        InMemoryRpcTransport::new("gym-test", move |_method, _params| {
            Err(RpcErrorPayload {
                code,
                message: msg.clone(),
            })
