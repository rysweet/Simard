//! Gym evaluation adapter connecting Simard to the amplihack-agent-eval suite.
//!
//! [`GymClient`] wraps a [`ServerTransport`] to communicate with the native
...
//!
//! If the adapter is unavailable or a call fails, errors are surfaced with
...
const ADAPTER_NAME: &str = "simard-gym-eval";
...
/// Adapter client for gym evaluations.
///
/// Wraps a [`ServerTransport`] to call the native gym evaluation transport.
/// Each method maps to a single adapter RPC call.
...
impl GymClient {
    ...
    pub fn list_scenarios(&self) -> SimardResult<Vec<GymScenario>> {
        ...
        unpack_server_response(ADAPTER_NAME, "gym.list_scenarios", response)
    }
    ...
    pub fn run_scenario(&self, scenario_id: &str) -> SimardResult<GymScenarioResult> {
        ...
        unpack_server_response(ADAPTER_NAME, "gym.run_scenario", response)
    }
    ...
    pub fn run_suite(&self, suite_id: &str) -> SimardResult<GymSuiteResult> {
        ...
        unpack_server_response(ADAPTER_NAME, "gym.run_suite", response)
    }
}
...
mod tests {
    ...
    fn fixed_result_transport(result: serde_json::Value) -> InMemoryServerTransport {
        InMemoryServerTransport::new("gym-test", move |_method, _params| Ok(result.clone()))
    }

    fn fixed_error_transport(code: i32, message: &str) -> InMemoryServerTransport {
        let msg = message.to_string();
        InMemoryServerTransport::new("gym-test", move |_method, _params| {
    ...
    fn list_scenarios_deserializes_response() {
        ...
        let result = adapter.list_scenarios().unwrap();
    ...
    fn run_scenario_deserializes_failure_with_error() {
        ...
        let transport = fixed_result_transport(result_json);
        let adapter = GymClient::new(Box::new(transport));
        let result = adapter.run_scenario("L12").unwrap();
        assert!(!result.success);
        assert_eq!(
    ...
    #[test]
    fn adapter_error_surfaces_with_context() {
        ...
        assert!(
            ...
            "error should name the adapter: {msg}"
        );
        assert!(
