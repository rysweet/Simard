//! Launch and manage adapter transports for live Simard operations.
//!
//! This module provides functions to create adapter transport instances for the
//! knowledge and gym adapters using [`NativeServerTransport`] for in-process
//! Rust execution.
//!
...
fn wrap_native(transport: NativeServerTransport) -> Box<dyn ServerTransport> {
...
/// Check adapter health and return it if healthy, or None with a log message.
fn check_health(name: &str, transport: &dyn ServerTransport) -> bool {
    match transport.health() {
        Ok(h) if h.healthy => true,
        Ok(_) => {
            eprintln!("[simard] {name} adapter reports unhealthy");
            false
        }
        Err(e) => {
            eprintln!("[simard] {name} adapter health check failed: {e}");
            false
        }
    }
}
...
/// Launch all adapters, returning None for any that fail (honest degradation).
///
/// Cognitive memory is now native — only knowledge and gym adapters are launched.
/// Both use the native Rust transport exclusively.
pub fn launch_all_adapters(
    _agent_name: &str,
    _state_root: &std::path::Path,
) -> (Option<KnowledgeClient>, Option<GymClient>) {
    let knowledge = match launch_knowledge_client_native() {
        Ok(b) => {
            eprintln!("[simard] knowledge adapter: using native Rust transport");
            Some(b)
        }
        Err(e) => {
            eprintln!("[simard] knowledge adapter launch FAILED — domain knowledge disabled: {e}");
            None
        }
    };
    ...
    let gym = match launch_gym_client_native() {
        Ok(b) => {
            eprintln!("[simard] gym adapter: using native Rust transport");
            Some(b)
        }
        Err(e) => {
            eprintln!("[simard] gym adapter launch FAILED — benchmarks disabled: {e}");
            None
        }
    };
...
/// Launch a knowledge adapter using the native Rust transport.
pub fn launch_knowledge_client_native() -> SimardResult<KnowledgeClient> {
    ...
    if !check_health("knowledge-native", wrapped.as_ref()) {
        return Err(crate::error::SimardError::ServerSpawnFailed {
            adapter: "knowledge-native".to_string(),
            reason: "native adapter unhealthy after init".to_string(),
        });
    }
    Ok(KnowledgeClient::new(wrapped))
}

/// Launch a gym adapter using the native Rust transport.
...
mod tests {
    ...
    fn launch_knowledge_client_native_succeeds() {
        ...
        assert!(
            result.is_ok(),
            "native knowledge adapter should launch: {:?}",
            result.err()
        );
    }
    ...
    fn launch_gym_client_native_succeeds() {
        ...
        assert!(
            result.is_ok(),
            "native gym adapter should launch: {:?}",
            result.err()
        );
    }
