...
mod reaper_tests {
    ...
    // INV-1 (issue #1779): `reap_zombies()` calls `waitpid(-1, …, WNOHANG)`,
    // which is *process-wide* — it will reap ANY <defunct> child of this test
    // binary, not just children spawned by the current test. That means peer
    // tests anywhere in the binary that also spawn-and-wait subprocesses can
    // race with these tests: a sibling's `wait()` can fail with ECHILD when
    // our `drain()` reaps its child first, or our assertion can see "got 0"
    // when a sibling reaps our zombie first.
    //
    // The fix is a named-key `#[serial]` cohort `subprocess_reaper` shared
    // across every test in the binary that either calls `reap_zombies()` or
    // spawns subprocesses it expects to `wait()` on later (see
    // `src/engineer_loop/tests_agent_spawn.rs`).
    //
    // The previous module-local `REAPER_TEST_LOCK` Mutex only serialized
    // *within* this module — it could not coordinate with sibling test
    // modules. Hence the move to `serial_test`.

    /// Drain any pre-existing zombies left behind by previous tests in the
    /// same process so each test starts from a clean baseline.
    fn drain() {
        for _ in 0..32 {
            if reap_zombies() == 0 {
                break;
