...
const ADAPTER_SOURCE_PREFIX: &str = "terminal-continuity-source=";
const ADAPTER_HANDOFF_PREFIX: &str = "terminal-continuity-handoff=";
const ADAPTER_WORKING_DIRECTORY_PREFIX: &str = "terminal-continuity-working-directory=";
const ADAPTER_COMMAND_COUNT_PREFIX: &str = "terminal-continuity-command-count=";
const ADAPTER_WAIT_COUNT_PREFIX: &str = "terminal-continuity-wait-count=";
const ADAPTER_LAST_OUTPUT_LINE_PREFIX: &str = "terminal-continuity-last-output-line=";

impl TerminalEngineerContext {
    ...
    pub fn from_engineer_evidence(
        ...
        let Some(continuity_source) =
            optional_evidence_value(evidence_records, ADAPTER_SOURCE_PREFIX)
        else {
            return Ok(None);
        };
        ...
        let handoff_file_name =
            required_evidence_value(evidence_records, ADAPTER_HANDOFF_PREFIX, "engineer read")?;
        ...
        Ok(Some(Self {
            ...
            working_directory: sanitize_terminal_text(required_evidence_value(
                evidence_records,
                ADAPTER_WORKING_DIRECTORY_PREFIX,
                "engineer read",
            )?),
            command_count: sanitize_terminal_text(required_evidence_value(
                evidence_records,
                ADAPTER_COMMAND_COUNT_PREFIX,
                "engineer read",
            )?),
            wait_count: sanitize_terminal_text(
                optional_evidence_value(evidence_records, ADAPTER_WAIT_COUNT_PREFIX).unwrap_or("0"),
            ),
            last_output_line: optional_evidence_value(
                evidence_records,
                ADAPTER_LAST_OUTPUT_LINE_PREFIX,
            )
    ...
    pub fn engineer_evidence_details(&self) -> Vec<String> {
        let mut details = vec![
            format!("{ADAPTER_SOURCE_PREFIX}{}", self.continuity_source),
            format!("{ADAPTER_HANDOFF_PREFIX}{}", self.handoff_file_name),
            format!(
                "{ADAPTER_WORKING_DIRECTORY_PREFIX}{}",
                self.working_directory
            ),
            format!("{ADAPTER_COMMAND_COUNT_PREFIX}{}", self.command_count),
            format!("{ADAPTER_WAIT_COUNT_PREFIX}{}", self.wait_count),
        ];
        if let Some(last_output_line) = &self.last_output_line {
            details.push(format!(
                "{ADAPTER_LAST_OUTPUT_LINE_PREFIX}{last_output_line}"
            ));
        }
        details
    }
    ...
    fn from_terminal_handoff(
        ...
        if handoff.session.is_none() {
            return Err(SimardError::InvalidHandoffSnapshot {
                ...
                reason: format!(
                    "{consumer_label} requires {handoff_file_name} to contain a persisted session snapshot before terminal continuity can be connected"
                ),
            });
        }
...
mod tests {
    ...
    fn test_engineer_evidence_details_basic() {
        let ctx = TerminalEngineerContext {
    ...
    fn test_from_engineer_evidence_returns_none_when_no_source() {
        ...
        let result = TerminalEngineerContext::from_engineer_evidence(&records).unwrap();
    ...
    fn test_from_engineer_evidence_rejects_invalid_source() {
        ...
        let result = TerminalEngineerContext::from_engineer_evidence(&records);
    ...
    fn test_from_engineer_evidence_valid() {
        ...
        let ctx = TerminalEngineerContext::from_engineer_evidence(&records)
            .unwrap()
            .unwrap();
