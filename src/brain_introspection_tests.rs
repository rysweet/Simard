...
//!     (`prune_superseded` over the IPC adapter is a `Ok(0)` no-op — calling it
...
use crate::ooda_reasoners::prompt_store::embedded_fallback;
...
impl CognitiveMemoryOps for HygieneStub {
    ...
    // daemon's IPC adapter (it is a `Ok(0)` no-op there — a silent-degradation
...
fn hygiene_performs_no_destructive_value_prune() {
    ...
    // daemon-side (it is a no-op over the IPC adapter — a silent-degradation
