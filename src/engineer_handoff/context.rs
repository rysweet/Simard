...
use super::types::{
    COMPATIBILITY_HANDOFF_FILE_NAME, EngineerHandoffContext, SHARED_DEFAULT_STATE_ROOT_SOURCE,
    SHARED_EXPLICIT_STATE_ROOT_SOURCE, ScopedHandoffMode, TERMINAL_HANDOFF_FILE_NAME,
};

const SOURCE_PREFIX: &str = "terminal-continuity-source=";
const HANDOFF_PREFIX: &str = "terminal-continuity-handoff=";
const WORKING_DIRECTORY_PREFIX: &str = "terminal-continuity-working-directory=";
const COMMAND_COUNT_PREFIX: &str = "terminal-continuity-command-count=";
const WAIT_COUNT_PREFIX: &str = "terminal-continuity-wait-count=";
const LAST_OUTPUT_LINE_PREFIX: &str = "terminal-continuity-last-output-line=";

impl EngineerHandoffContext {
    ...
    pub fn from_engineer_evidence(
        ...
        let Some(continuity_source) = optional_evidence_value(evidence_records, SOURCE_PREFIX)
        else {
            return Ok(None);
        };
        ...
        let handoff_file_name =
            required_evidence_value(evidence_records, HANDOFF_PREFIX, "engineer read")?;
        ...
        Ok(Some(Self {
            ...
            command_count: sanitize_terminal_text(required_evidence_value(
                evidence_records,
                COMMAND_COUNT_PREFIX,
                "engineer read",
            )?),
            wait_count: sanitize_terminal_text(
                optional_evidence_value(evidence_records, WAIT_COUNT_PREFIX).unwrap_or("0"),
            ),
            last_output_line: optional_evidence_value(evidence_records, LAST_OUTPUT_LINE_PREFIX)
    ...
    pub fn engineer_evidence_details(&self) -> Vec<String> {
        let mut details = vec![
            format!("{SOURCE_PREFIX}{}", self.continuity_source),
            format!("{HANDOFF_PREFIX}{}", self.handoff_file_name),
            format!("{WORKING_DIRECTORY_PREFIX}{}", self.working_directory),
            format!("{COMMAND_COUNT_PREFIX}{}", self.command_count),
            format!("{WAIT_COUNT_PREFIX}{}", self.wait_count),
        ];
        if let Some(last_output_line) = &self.last_output_line {
            details.push(format!("{LAST_OUTPUT_LINE_PREFIX}{last_output_line}"));
        }
        details
    }
...
mod tests {
    ...
    fn test_from_engineer_evidence_returns_none_when_no_source() {
        ...
        let result = EngineerHandoffContext::from_engineer_evidence(&records).unwrap();
    ...
    fn test_from_engineer_evidence_rejects_invalid_source() {
        ...
        let result = EngineerHandoffContext::from_engineer_evidence(&records);
    ...
    fn test_from_engineer_evidence_valid() {
        ...
        let ctx = EngineerHandoffContext::from_engineer_evidence(&records)
            .unwrap()
            .unwrap();
