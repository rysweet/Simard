...
fn mock_adapter() -> CognitiveMemoryAdapter {
    let transport = InMemoryServerTransport::new("test-meeting", |method, _params| match method {
...
fn start_and_close_meeting_round_trip() {
    ...
    let mut session = start_meeting("Sprint planning", &adapter).unwrap();
...
fn rejects_empty_topic() {
    let adapter = mock_adapter();
    let err = start_meeting("", &adapter).unwrap_err();
...
fn rejects_zero_priority_action_item() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Check", &adapter).unwrap();
...
fn edit_decision_description() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Edit test", &adapter).unwrap();
    record_decision(
        &mut session,
        MeetingDecision {
...
fn remove_decision() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Delete test", &adapter).unwrap();
    record_decision(
        &mut session,
        MeetingDecision {
...
fn add_question_to_session() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Question test", &adapter).unwrap();
...
fn add_question_rejects_closed_meeting() {
    let adapter = mock_adapter();
    let session = start_meeting("Q", &adapter).unwrap();
    let mut closed = close_meeting(session, &adapter).unwrap();
...
fn edit_question() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Edit Q", &adapter).unwrap();
...
fn remove_question() {
    let adapter = mock_adapter();
    let mut session = start_meeting("Delete Q", &adapter).unwrap();
