//! Construct an [`OodaContext`] from a `state_root` path so that recipe
//! steps (which run as short-lived helper-bin invocations) can instantiate
//! the same adapters the long-lived OODA daemon uses.
//!
//! Strategy:
//!
...
//!    [`crate::server_launcher`]. These are in-process and incur negligible
...
//! helper-bin model. Both paths share `server_launcher` for the native
...
pub fn connect_memory(state_root: &Path) -> SimardResult<Box<dyn CognitiveMemoryOps>> {
        ...
        && let Ok(remote) = RemoteCognitiveMemory::connect(&socket_path)
    {
        ...
        // `Box<dyn CognitiveMemoryOps>` shape expected by OodaContext.
...
/// Build an [`OodaContext`] suitable for stateless helper-bin invocations.
///
...
/// session).
pub fn context_from_state_root(state_root: &Path) -> SimardResult<OodaContext> {
    ...
    Ok(OodaContext {
        ...
        act_reasoner: std::sync::Arc::new(crate::ooda_reasoners::DeterministicFallbackActReasoner),
