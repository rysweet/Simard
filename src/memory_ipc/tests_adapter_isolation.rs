...
//! adapter-launcher's interaction with the new per-state-root socket
//! resolution.
//!
//! The contract under test is the end-to-end hermeticity property:
//! when a test sets `SIMARD_STATE_ROOT=<tempdir>` and unsets
//! `SIMARD_MEMORY_SOCKET`, [`launch_writer_adapter`] must NOT connect
...
//! `launch_writer_adapter` / `open_reader_adapter` to call
...
use super::{MEMORY_SOCKET_ENV, launch_writer_adapter, open_reader_adapter};
...
fn launch_writer_adapter_with_tempdir_state_root_does_not_touch_home_simard() {
    ...
    let adapter = launch_writer_adapter(root)
        .expect("launch_writer_adapter must succeed against a fresh TempDir state root");
    ...
    assert!(
        ...
        "after a writer adapter round-trip against state_root={}, at \
    ...
    // Property 2: the adapter must NOT have written to ~/.simard. Inspect
    ...
    if let Ok(live_adapter) = open_reader_adapter(&live_state) {
        let hits = live_adapter
            .ops()
        ...
        assert!(
            ...
            "TempDir-rooted writer must NOT have written into \
             ...
             fixture leak — the adapter connected to the live \
...
#[test]
#[serial(cognitive_memory)]
fn writer_and_reader_adapter_round_trip_within_hermetic_state_root() {
    ...
    {
        let writer =
            launch_writer_adapter(root).expect("writer adapter against hermetic state root");
        writer
            .ops()
            .store_fact(
                &tag,
    ...
    let reader = open_reader_adapter(root).expect("reader adapter against hermetic state root");
    let hits = reader
        .ops()
        .search_facts(&tag, 4, 0.0)
