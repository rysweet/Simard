...
pub(crate) async fn memory_history() -> Json<Value> {
    ...
    // `open_reader_adapter` so the daemon's IPC writer serves embedded reads.
    let stats_result =
...
pub(crate) async fn memory_recent() -> Json<Value> {
    let state_root = resolve_state_root();
    let total = open_reader_adapter(&state_root)
...
/// available through the trait (routed via `open_reader_adapter`), so those are
...
pub(crate) async fn memory_graph() -> Json<Value> {
    let state_root = resolve_state_root();
    let stats = open_reader_adapter(&state_root)
        .and_then(|reader| reader.ops().get_statistics())
        .unwrap_or_default();
    Json(json!({
        "nodes": [],
