...
fn circuit_breaker_passes_through_on_success() {
    ...
    let inner = InMemoryRpcTransport::new("cb-ok", |method, params| match method {
...
fn circuit_breaker_opens_after_repeated_transport_failures() {
    use crate::rpc_circuit_breaker::{CircuitBreakerConfig, CircuitBreakerTransport, CircuitState};
    use std::time::Duration;

    let inner = InMemoryRpcTransport::new("cb-fail", |_method, _params| {
        Err(crate::rpc::RpcErrorPayload {
            code: crate::rpc::RPC_ERROR_TRANSPORT,
    ...
    let _ = cb.call(crate::rpc::RpcRequest {
...
fn circuit_breaker_recovers_after_cooldown() {
    ...
    let inner = InMemoryRpcTransport::new("cb-recover", move |_method, _params| {
...
fn empty_facts_response() {
    let transport = InMemoryRpcTransport::new("empty-facts", |method, _params| match method {
...
fn empty_working_slots_response() {
    let transport = InMemoryRpcTransport::new("empty-working", |method, _params| match method {
...
fn consolidate_episodes_with_present_id() {
    let transport = InMemoryRpcTransport::new("consolidate-ok", |method, _params| match method {
...
fn search_facts_respects_params() {
    let transport = InMemoryRpcTransport::new("search-params", |method, params| match method {
        "memory.search_facts" => {
