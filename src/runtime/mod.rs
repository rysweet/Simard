...
pub struct RuntimeKernel {
    ...
    cognitive_memory: Option<Box<dyn CognitiveMemoryOps>>,
}
...
impl RuntimeKernel {
    ...
    pub fn set_cognitive_client(&mut self, memory: Box<dyn CognitiveMemoryOps>) {
        self.cognitive_memory = Some(memory);
    }

    /// Flush any pending memory memory writes (retries failed writes).
