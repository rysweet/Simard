...
impl RuntimeKernel {
    pub(super) fn execute_session(&mut self, objective: String) -> SimardResult<SessionOutcome> {
        ...
        if let Some(adapter) = &self.cognitive_adapter {
        ...
        // Flush any pending adapter writes before final persistence so that
        ...
        if let Some(adapter) = &self.cognitive_adapter {
            ...
            crate::memory_consolidation::consolidation_persistence(&session.id, &**adapter)?;
            crate::memory_consolidation::persistence_memory_operations(&session.id, &**adapter)?;
