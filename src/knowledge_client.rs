//! Knowledge Graph Pack adapter for querying agent-kgpacks from Simard.
//!
//! This module wraps a [`ServerTransport`] to provide a typed interface for
...
//! the actual pack registry and query interactions.
...
/// Name used in error messages for this adapter.
const ADAPTER_NAME: &str = "knowledge";
...
/// Typed client for the knowledge graph pack adapter.
///
/// All methods delegate to the underlying [`ServerTransport`] using the
/// adapter JSON-line protocol. The native Rust transport maps these to
...
impl KnowledgeClient {
    /// Create a new knowledge adapter wrapping the given transport.
    ...
    pub fn query(
        ...
        unpack_server_response(ADAPTER_NAME, "knowledge.query", response)
    }
    ...
    pub fn list_packs(&self) -> SimardResult<Vec<KnowledgePackInfo>> {
        ...
        let wrapper: ListPacksResponse =
            unpack_server_response(ADAPTER_NAME, "knowledge.list_packs", response)?;
        Ok(wrapper.packs)
    }
    ...
    pub fn pack_info(&self, pack_name: &str) -> SimardResult<KnowledgePackInfo> {
        ...
        unpack_server_response(ADAPTER_NAME, "knowledge.pack_info", response)
    }

    /// Check whether the adapter server is alive and responsive.
    pub fn health(&self) -> SimardResult<ServerHealth> {
    ...
    fn call(
        ...
        method: &str,
        params: Value,
    ) -> SimardResult<crate::server_transport::ServerResponse> {
...
mod tests {
    ...
    use crate::server_transport::{SERVER_ERROR_METHOD_NOT_FOUND, ServerErrorPayload};

    fn mock_transport() -> InMemoryServerTransport {
        InMemoryServerTransport::new("knowledge-test", |method, params| match method {
    ...
    fn query_returns_typed_result() {
        let adapter = KnowledgeClient::new(Box::new(mock_transport()));
        let result = adapter
    ...
    fn query_unknown_pack_returns_error() {
        let adapter = KnowledgeClient::new(Box::new(mock_transport()));
        let result = adapter.query("nonexistent", "anything", 5);
