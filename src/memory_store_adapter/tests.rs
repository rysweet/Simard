...
fn hydration_loads_records_from_local_store() {
    ...
    // Step 2: create a CognitiveClientMemoryStore that reads the same path.
    let transport = InMemoryRpcTransport::new("test-hydrate", |method, _params| match method {
...
fn hydration_with_empty_local_store_starts_empty() {
    ...
    let transport =
        InMemoryRpcTransport::new("test-hydrate-empty", |method, _params| match method {
...
fn hydration_plus_new_put_merge_correctly() {
    ...
    let transport = InMemoryRpcTransport::new("test-merge", |method, _params| match method {
...
fn list_for_session_includes_hydrated_records() {
    ...
    let transport = InMemoryRpcTransport::new("test-session", |method, _params| match method {
