...
#[test]
fn descriptor_identifies_cognitive_adapter() {
    let store = test_store();
    let desc = store.descriptor();
    assert!(desc.identity.contains("cognitive-adapter"));
}
...
fn hydration_loads_records_from_local_store() {
    ...
    // Step 2: create a CognitiveMemoryStoreAdapter that reads the same path.
    let transport = InMemoryServerTransport::new("test-hydrate", |method, _params| match method {
    ...
    let store = CognitiveMemoryStoreAdapter::new(adapter, &path).unwrap();
...
fn hydration_with_empty_local_store_starts_empty() {
    ...
    let transport =
        InMemoryServerTransport::new("test-hydrate-empty", |method, _params| match method {
...
fn hydration_plus_new_put_merge_correctly() {
    ...
    let transport = InMemoryServerTransport::new("test-merge", |method, _params| match method {
...
fn list_for_session_includes_hydrated_records() {
    ...
    let transport = InMemoryServerTransport::new("test-session", |method, _params| match method {
...
#[test]
fn local_read_hits_without_adapter() {
    // When local index has records, list() returns them without calling adapter.
    let store = test_store();
    store
...
fn write_through_updates_local_cache() {
    ...
    // without needing an adapter call.
    let store = test_store();
