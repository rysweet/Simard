...
fn local_miss_triggers_bridge_query() {
    ...
    let sid = Uuid::nil();
    let transport =
        InMemoryRpcTransport::new("test-bridge-query", move |method, _params| match method {
    ...
    let store = CognitiveClientMemoryStore::new(bridge, path.clone()).unwrap();
...
fn hydrate_from_client_merges_new_records() {
    let transport =
        InMemoryRpcTransport::new("test-bridge-hydrate", |method, _params| match method {
