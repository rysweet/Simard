//! [`CognitiveMemoryStoreAdapter`] — the adapter between `MemoryStore` and cognitive memory.
...
use super::{ADAPTER_READ_MAX_RETRIES, ADAPTER_RETRY_BACKOFF_MS, STORE_NAME};

/// `MemoryStore` implementation backed by cognitive memory via Python adapter.
///
...
///
/// Dual-writes to both the cognitive adapter and a local `FileBackedMemoryStore` for persistence.
pub struct CognitiveMemoryStoreAdapter {
    adapter: Box<dyn CognitiveMemoryOps>,
    ...
    /// Keys whose adapter write failed — `sync_pending()` retries these.
    pending_adapter_keys: Mutex<Vec<String>>,
    descriptor: BackendDescriptor,
}

impl CognitiveMemoryStoreAdapter {
    pub fn new(
        adapter: impl CognitiveMemoryOps + 'static,
        ...
        let mut store = Self {
            ...
            pending_adapter_keys: Mutex::new(Vec::new()),
            descriptor: BackendDescriptor::for_runtime_type::<Self>(
                "memory::cognitive-adapter",
                "runtime-port:memory-store:cognitive-adapter",
                Freshness::now()?,
            ),
            local_store,
        };
    ...
    fn hydrate_from_file_store(&mut self) -> SimardResult<()> {
        ...
        if hydrated > 0 {
            eprintln!("[simard] cognitive-adapter: hydrated {hydrated} records from file store");
        }
        Ok(())
    }

    /// Pull facts from the cognitive adapter (Python subprocess) and merge into
    ...
    ///
    /// Adapter errors propagate via `?` per PHILOSOPHY.md.
    pub fn hydrate_from_adapter(&self) -> SimardResult<()> {
        ...
        if hydrated > 0 {
            eprintln!("[simard] cognitive-adapter: hydrated {hydrated} records from adapter");
        }
        Ok(())
    }

    /// Search facts via the cognitive adapter with retry logic.
    fn search_facts_with_retry(
        ...
        for attempt in 0..=ADAPTER_READ_MAX_RETRIES {
            match self.adapter.search_facts(query, limit, min_confidence) {
                Ok(facts) => return Ok(facts),
                Err(e) => {
                    if attempt < ADAPTER_READ_MAX_RETRIES {
                        eprintln!(
                            "[simard] cognitive-adapter: search_facts retry {}/{} after error: {e}",
                            attempt + 1,
                            ADAPTER_READ_MAX_RETRIES
                        );
    ...
    /// Query the adapter for records matching a scope, converting facts back to
    /// `MemoryRecord`s. Used when the local index has no results.
    fn adapter_list(&self, scope: MemoryScope) -> SimardResult<Vec<MemoryRecord>> {
    ...
    /// Retry adapter writes for records that were persisted to the local file
    /// store but failed to reach the cognitive adapter. Returns the number of
    /// successfully synced records.
    pub fn sync_pending(&self) -> usize {
        let keys: Vec<String> = {
            let Ok(pending) = self.pending_adapter_keys.lock() else {
                return 0;
            };
            pending.clone()
        };
        ...
        for key in &keys {
            if let Some(record) = records_map.get(key) {
                match self.store_as_fact(record) {
                    ...
                    Err(e) => {
                        eprintln!(
                            "[simard] cognitive-adapter: sync_pending retry failed \
                             for key {:?}: {e}",
                            key,
                        );
        ...
        if let Ok(mut pending) = self.pending_adapter_keys.lock() {
            *pending = still_pending;
        }
        if synced > 0 {
            eprintln!("[simard] cognitive-adapter: sync_pending synced {synced} records");
        }
        synced
    }
...
impl MemoryStore for CognitiveMemoryStoreAdapter {
    ...
    fn put(&self, record: MemoryRecord) -> SimardResult<()> {
        ...
        // Write to cognitive adapter — failure is an error, not silently swallowed.
    ...
    fn list(&self, scope: MemoryScope) -> SimardResult<Vec<MemoryRecord>> {
        ...
        // Local miss — query adapter for cross-session data.
        drop(records); // release lock before adapter call
        self.adapter_list(scope)
    }
