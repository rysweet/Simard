...
//! snapshots the live store through the adapter (inherently the migrated path),
...
///
/// Steps:
/// 1. Snapshot the live cognitive store (`adapter`) plus the file-backed records
...
pub fn run_verified_backup(
    ...
    let manifest = backup_memory_verified(adapter, &file_store, BACKUP_AGENT, &config)?;
...
mod tests {
    ...
    fn run_verified_backup_produces_valid_backup() {
        ...
        let adapter = LibraryCognitiveMemory::in_memory().unwrap();
        adapter
        ...
        let manifest = run_verified_backup(&adapter, state_root).expect("verified backup");
    ...
    fn run_verified_backup_on_empty_store_is_valid() {
        ...
        let manifest = run_verified_backup(&adapter, tmp.path()).expect("verified empty backup");
