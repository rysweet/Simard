...
#[test]
fn store_and_search_fact_via_adapter() {
...
fn record_sensory_returns_node_id() {
    let adapter = mock_adapter();
    let id = adapter.record_sensory("visual", "raw pixels", 60).unwrap();
...
fn prune_expired_sensory_returns_count() {
    let adapter = mock_adapter();
    let count = adapter.prune_expired_sensory().unwrap();
    assert_eq!(count, 0);
}
...
fn push_and_get_working_round_trip() {
    ...
    let slots = adapter.get_working("task-1").unwrap();
...
fn clear_working_returns_count() {
    let adapter = mock_adapter();
    let count = adapter.clear_working("task-1").unwrap();
    assert_eq!(count, 1);
}
...
fn consolidate_episodes_returns_none_when_insufficient() {
    let adapter = mock_adapter();
    let result = adapter.consolidate_episodes(10).unwrap();
...
fn recall_procedure_returns_list() {
    let adapter = mock_adapter();
    let procs = adapter.recall_procedure("build", 5).unwrap();
...
fn check_triggers_returns_empty_vec() {
    let adapter = mock_adapter();
    let triggered = adapter.check_triggers("some content").unwrap();
...
#[test]
fn store_fact_propagates_adapter_error() {
...
#[test]
fn search_facts_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.search_facts("q", 10, 0.0);
    assert!(result.is_err());
}

#[test]
fn record_sensory_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.record_sensory("audio", "data", 30);
    assert!(result.is_err());
}

#[test]
fn get_working_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.get_working("task-1");
    assert!(result.is_err());
}

#[test]
fn get_statistics_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.get_statistics();
    assert!(result.is_err());
}

#[test]
fn consolidate_episodes_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.consolidate_episodes(5);
    assert!(result.is_err());
}

#[test]
fn recall_procedure_propagates_adapter_error() {
    let adapter = error_adapter();
    let result = adapter.recall_procedure("build", 5);
    assert!(result.is_err());
}
