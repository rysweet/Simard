...
pub fn advance_goal_with_subordinate(
    ...
    match check_heartbeat(&handle, &*adapters.memory) {
        ...
        Ok(HeartbeatStatus::Stale { seconds_since }) => {
            ...
            } else if let Err(e) = save_goal_board(&state.active_goals, &*adapters.memory) {
                eprintln!(
        ...
        Ok(HeartbeatStatus::Dead) => {
            ...
            if let Err(e) = save_goal_board(&state.active_goals, &*adapters.memory) {
                eprintln!(
                    "[simard] OODA advance_goal FAILED to persist goal board after \
