...
fn counting_adapter() -> (CognitiveMemoryAdapter, Arc<AtomicU32>) {
    ...
    let transport = InMemoryServerTransport::new("test", move |method, _params| {
...
fn intake_records_sensory_working_and_episode() {
    let (adapter, count) = counting_adapter();
    intake_memory_operations("build feature X", &test_session_id(), &adapter).unwrap();
...
fn preparation_returns_empty_context_when_memory_empty() {
    let (adapter, _) = counting_adapter();
    let ctx =
        preparation_memory_operations("build feature X", &test_session_id(), &adapter).unwrap();
...
fn reflection_stores_transcript_and_facts() {
    ...
    reflection_memory_operations("transcript...", &facts, &test_session_id(), &adapter).unwrap();
...
fn execution_truncates_multibyte_utf8_safely() {
    ...
    // Must not panic.
    execution_memory_operations(&cjk_output, &test_session_id(), &adapter).unwrap();
}

#[test]
fn execution_does_not_truncate_short_output() {
    let (adapter, _) = counting_adapter();
    execution_memory_operations("short", &test_session_id(), &adapter).unwrap();
}

#[test]
fn persistence_clears_working_and_prunes() {
    let (adapter, count) = counting_adapter();
    persistence_memory_operations(&test_session_id(), &adapter).unwrap();
...
fn consolidation_intake_returns_zero_when_no_prior_facts() {
    let (adapter, count) = counting_adapter();
    let hydrated = consolidation_intake(&test_session_id(), "test-objective", &adapter).unwrap();
...
fn consolidation_intake_with_facts_pushes_to_working_memory() {
    let call_count = Arc::new(AtomicU32::new(0));
    let counter = call_count.clone();
    let transport = InMemoryServerTransport::new("test-intake", move |method, _params| {
        counter.fetch_add(1, Ordering::SeqCst);
        match method {
...
fn consolidation_persistence_flushes_and_consolidates() {
    let (adapter, count) = counting_adapter();
    consolidation_persistence(&test_session_id(), &adapter).unwrap();
...
/// Build an adapter whose `search_facts("goal-store:record", ...)` returns
...
fn goal_dedup_adapter() -> CognitiveMemoryAdapter {
    use crate::goals::GoalRecord;
    let transport = InMemoryServerTransport::new("goal-dedup", move |method, params| {
        match method {
            "memory.search_facts" => {
...
fn preparation_deduplicates_goal_facts_by_slug_keeping_latest() {
    // The adapter returns 3 goal facts: 2 revisions of "alpha" and 1 "beta".
    // After dedup, only the latest "alpha" (node_id n_0002) and "beta" should
    // appear in relevant_facts.
    let adapter = goal_dedup_adapter();
    let ctx =
        preparation_memory_operations("unrelated objective", &test_session_id(), &adapter).unwrap();
...
fn preparation_does_not_include_unparseable_goal_facts() {
    // An adapter that returns one valid goal fact and one with malformed JSON.
    let transport = InMemoryServerTransport::new("bad-json", move |method, params| match method {
        "memory.search_facts" => {
            let query = params.get("query").and_then(|v| v.as_str()).unwrap_or("");
            if query == "goal-store:record" {
                Ok(json!({
...
fn preparation_uses_goal_store_list_limit_not_hardcoded_20() {
    ...
    let transport = InMemoryServerTransport::new("limit-check", move |method, params| {
        match method {
            "memory.search_facts" => {
                let query = params.get("query").and_then(|v| v.as_str()).unwrap_or("");
                if query == "goal-store:record" {
    ...
    let adapter = CognitiveMemoryAdapter::new(Box::new(transport));
    preparation_memory_operations("check limit", &test_session_id(), &adapter).unwrap();
...
/// Build a counting adapter that satisfies every call needed by the
/// persistence phase *and* the snapshot save (search_facts + recall_procedure).
fn persistence_capable_adapter() -> CognitiveMemoryAdapter {
    let transport = InMemoryServerTransport::new("snapshot-fail", |method, _params| match method {
        "memory.consolidate_episodes" => Ok(json!({"id": null})),
...
fn persistence_propagates_snapshot_save_error_when_dir_is_a_file() {
    ...
    let adapter = persistence_capable_adapter();
...
/// data is actually queryable, unlike the counting adapter which only
/// counts calls.
#[test]
...
/// Build an adapter that captures search_facts query strings and returns
/// per-fragment results with controllable node_ids for dedup testing.
fn compound_objective_adapter(
    ...
    let transport =
        InMemoryServerTransport::new("compound-obj", move |method, params| match method {
            "memory.search_facts" => {
                let query = params.get("query").and_then(|v| v.as_str()).unwrap_or("");
...
fn preparation_splits_compound_objective_into_separate_searches() {
    ...
    let adapter = compound_objective_adapter(queries.clone(), std::collections::HashMap::new());
...
fn preparation_single_fragment_objective_produces_one_search() {
    ...
    let _ctx = preparation_memory_operations("fix auth bug", &test_session_id(), &adapter).unwrap();
...
fn preparation_deduplicates_split_results_by_node_id() {
    ...
    let adapter = compound_objective_adapter(queries, facts_map);
    let ctx =
        preparation_memory_operations("goal A; goal B", &test_session_id(), &adapter).unwrap();
