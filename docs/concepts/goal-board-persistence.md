...
|-----------|----------|-------------|
| Read | `load_goal_board(bridge.ops())` | `ReaderAdapter` (cheap, never contends with daemon writer lock) |
| Write | `save_goal_board(&board, bridge.ops())` | `WriterAdapter` (prefers daemon IPC when available, else takes the local writer lock; fails synchronously if no writer is obtainable) |
...
`launch_writer_adapter`'s tier 1, so writes are serialized by the daemon.
When no daemon is running, the writer bridge takes the local LadybugDB
writer lock directly.

### Bridge ladders

`launch_writer_adapter` today resolves two writer-bearing tiers in order,
...
3. **Read-only fallback (current)** — wrap a read-only handle as a
   `WriterAdapter`. **The issue-#1590 follow-up removes this tier**; once
...
`open_reader_adapter` resolves two tiers:
...
See the [helper reference](../reference/cognitive-memory-adapter-helpers.md)
...
| Dashboard goals API | `src/operator_commands_dashboard/goals.rs` | `launch_writer_adapter` (writes), `open_reader_adapter` (reads) | `load_goal_board` | `save_goal_board` (×6 mutation handlers) | The in-process Arc shortcut keeps mutation handlers from going through Unix-socket IPC against the same process |
| Dashboard workboard | `src/operator_commands_dashboard/workboard.rs` | `open_reader_adapter` | `load_goal_board` | — | Read-only |
| Dashboard current work | `src/operator_commands_dashboard/current_work.rs` | `open_reader_adapter` | `load_goal_board` | — | Read-only |
| Dashboard metrics panel | `src/operator_commands_dashboard/metrics.rs` | `open_reader_adapter` | `load_goal_board` | — | Reports `{ source: "cognitive-memory:goal-board:snapshot", count: N }` |
| Meeting goal curation | `src/operator_commands_meeting/goal_curation.rs` | `open_reader_adapter` (reads), `launch_writer_adapter` (mutations) | `load_goal_board` + `active_goals_as_records` | `save_goal_board` | Replaces `FileBackedGoalStore` |
| Meeting improvement curation | `src/operator_commands_meeting/improvement_curation.rs` | `launch_writer_adapter` | `load_goal_board` + `active_goals_as_records` | `save_goal_board` | Replaces `FileBackedGoalStore` |
| Engineer loop | `src/engineer_loop/mod.rs` | `launch_writer_adapter` (used because the load step also performs the legacy `goal_records.json` migration write-back) | `load_goal_board` + `active_goals_as_records` | — | Reads top 5 active goals as `GoalRecord`s |
...
- **Bridge writes when no writer can be acquired**: `launch_writer_adapter`
