...
| Value | Behavior |
|---|---|
| Unset, or any value other than `off` (case-insensitive) | `OodaContext.progress_evidence` is wired to `RecipeProgressChecker`, which invokes a recipe-based LLM agent to verify that proposed progress claims are coherent with the goal's plan and WIP artifacts. |
| `off` (case-insensitive) | `OodaContext.progress_evidence` is wired to `NoopProgressEvidenceChecker`. Every progress claim is accepted. **No `"goal progress accepted:"` or `"brain hallucination detected:"` audit episodes are emitted.** |
