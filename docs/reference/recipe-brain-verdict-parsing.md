...
`escape_json_string_invalid_escapes`; and `normalize_python_json_literals`) are
...
pub fn extract_and_parse_json<T: serde::de::DeserializeOwned>(raw: &str) -> Option<T> {
    ...
    match serde_json::from_str::<T>(&payload) {
        ...
        // strip_json_trailing_commas + normalize_python_json_literals; each is a
...
`true`/`false`/`null`. The
five views cannot interfere: the two string views only touch bytes inside string
literals, comment-stripping, comma-stripping, and literal-normalisation only touch
bytes outside them.
...
leave it a raw control byte. Literal-normalisation runs **last**, after comments
are already stripped, so its own `in_string` scan is aligned with the real string
boundaries and only rewrites the exact tokens `True`/`False`/`None` — a longer
identifier run (`Truthy`, `Nonexistent`, `xNone`) is left verbatim. All five are
string-literal aware, so a comma,
control byte, backslash, `//`/`/*` sequence, or a `True`/`False`/`None` word that
...
capitalised word that is not exactly `True`/`False`/`None` still
...
one comma, one literal newline, one lone backslash, one annotation comment, or one
bare literal is
