---
title: Server Wire Protocol Reference
...
# Server Wire Protocol Reference

All Simard adapters communicate via newline-delimited JSON on stdin (requests) and stdout (responses). This document specifies every method, its parameters, and its response shape.
...
> The gym bridge (`simard-gym-eval`) runs the progressive L1–L12 evaluation
