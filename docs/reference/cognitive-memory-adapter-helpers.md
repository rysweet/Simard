...
description: Reference for launch_writer_adapter and open_reader_adapter — the canonical entry points for obtaining a CognitiveMemoryOps adapter, including the planned in-process Arc shortcut and strict no-silent-degradation contract.
...
related:
  ...
  - ./server-wire-protocol.md
---

# Cognitive memory bridge helpers

> **Status: partially shipped + design.** The two helpers
> (`launch_writer_adapter`, `open_reader_adapter`) and their two-tier
...
|--------|---------|----------|
| `launch_writer_adapter` | `SimardResult<WriterAdapter>` | Anything that may write — dashboard mutation handlers (`promote_goal`, `demote_goal`, `dismiss_goal`, …), meeting REPL flows, restore CLI |
| `open_reader_adapter` | `SimardResult<ReaderAdapter>` | Read-only consumers — dashboard read handlers (`workboard`, `current_work`, `metrics`, GET goals), engineer-loop top-5 read, inspection tools |
...
The two helpers return distinct types — `WriterAdapter` and `ReaderAdapter` —
...
A `ReaderAdapter` is intentionally **not** convertible into a `WriterAdapter`.
Callers that hold a `ReaderAdapter` and discover they need to write must
re-call `launch_writer_adapter` — at which point any acquisition failure is
reported up-front.
...
`launch_writer_adapter`, the launcher checks the `OnceLock` first. On a hit,
it wraps the `Arc` in a `WriterAdapter` and returns immediately — no
...
Tier 3 (read-only fallback wrapped as `WriterAdapter`) is **removed** in the
...
`launch_writer_adapter` success as "we have a writer". When the helper
...
`store_fact` call returns `ServerTransportError`), and the handler's HTTP
...
`WriterAdapter`'s constructor calls `assert!(!ops.is_read_only(), …)` — an
...
virtual call per `WriterAdapter` construction.

**Example — dashboard write handler (post-fix)**

```rust
use simard::goal_curation::{load_goal_board, save_goal_board};
use simard::memory_ipc::{launch_writer_adapter, default_state_root};

let state_root = default_state_root();
let bridge = launch_writer_adapter(&state_root)?;     // Err if no writer
...
After the fix, the `?` on `launch_writer_adapter` is load-bearing: where
...
The returned `ReaderAdapter` does not carry a write capability in its type.
...
at runtime with `ServerTransportError`. Callers should use `WriterAdapter`
when they intend to write — see "Typed bridge wrappers" above for the
rationale.

`open_reader_adapter` is **not** affected by the issue-#1590 follow-up; its
ladder is unchanged.
...
use simard::memory_ipc::{open_reader_adapter, default_state_root};

let bridge = open_reader_adapter(&default_state_root())?;
...
[`launch_writer_adapter`](#launch_writer_adapter) and the dashboard /
...
| Site | Helper | Status |
|------|--------|--------|
| `engineer_loop::engineer_loop_run_inner` (top-5 read) | `launch_writer_adapter` | shipped (uses writer for legacy migration write-back; will move to `open_reader_adapter` when migration is removed) |
| Meeting REPL goal-curation flows | `open_reader_adapter` / `launch_writer_adapter` | shipped |
| Meeting REPL improvement-curation flows | `launch_writer_adapter` | shipped |
| Operator dashboard goals API (mutations) | `launch_writer_adapter` | shipped |
| Operator dashboard goals API (GET) | `open_reader_adapter` | shipped |
| Operator dashboard workboard / current_work / metrics | `open_reader_adapter` | shipped |
...
- [Cognitive memory bridge wire protocol](./server-wire-protocol.md) —
