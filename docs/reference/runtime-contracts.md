...
- `latest_handoff.json` is used only as a compatibility client when the engineer-scoped handoff is absent
...
- `latest_handoff.json` is used only as a compatibility client when the terminal-scoped handoff is absent
- the rendered output includes which handoff file was used
...
- the rendered output also includes explicit engineer-next-step guidance so the client into repo-grounded engineer work stays operator-visible
...
The builtin identities currently advertised by the loader are `simard-engineer`, `simard-meeting`, `simard-goal-curator`, `simard-improvement-curator`, `simard-gym`, `simard-atelier`, and the composite `simard-composite-engineer`. All of them accept `local-harness`, `rusty-clawd`, and `copilot-sdk`; `simard-engineer` and `simard-atelier` additionally accept `terminal-shell` for the local terminal-backed path (`simard-atelier` needs a shell base type to drive its CAD tooling).
