...
The builtin identities currently advertised by the loader are `simard-engineer`, `simard-meeting`, `simard-goal-curator`, `simard-improvement-curator`, `simard-gym`, `simard-atelier` (industrial & furniture design), and the composite `simard-composite-engineer`. All of them accept `local-harness`, `rusty-clawd`, and `copilot-sdk`; `simard-engineer` and `simard-atelier` additionally accept `terminal-shell` for the local terminal-backed path.
