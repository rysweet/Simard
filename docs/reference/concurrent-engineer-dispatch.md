...
non-`LaunchSession`/`SafeUpdate` action acquired a **global `adapters`+`state`
...
**single shared `adapters.session`** plus the engineer spawn. So all
...
`dispatch_actions_bounded(actions, adapters, state, max_concurrency)` partitions
...
|-------|---------|-------------|----------|
| **Phase 1 — serialized** | `LaunchSession` and `SafeUpdate` (independent, no shared state); **assigned-goal heartbeat** `AdvanceGoal`; every non-`AdvanceGoal` kind | One thread per action; the non-independent kinds take a short global `adapters`+`state` lock | Unchanged from before — today's behavior over this subset. |
...
threads shared one `adapters.session`. Phase 2 fixes this with a session
**factory**:
...
- `OodaContext.session_factory: Option<Arc<dyn OrchestratorSessionFactory>>`.
...
- When `session_factory` is `None` (tests and non-daemon callers), Phase 2
  falls back to the single shared `adapters.session` **under a lock**, i.e.
