...
ecosystem's **single source of truth** — Simard's identity-curated `governed_repos`
roster (see [Governed-roster resolution](./ecosystem-roster-resolution.md)) — via
the same `resolve_governed_roster` the Overseer's `ecosystem-observe` rail uses
(note `amplihack` → `amplihack-rs` on GitHub). There is no second hardcoded roster
to drift: adding a repo to Simard's durable roster (a one-line data edit, or an
agentic curation, that survives self-deploys) extends this sweep on the next run.
An empty or all-invalid roster is a fail-loud error, never a silently empty sweep
that would report the fleet green.
