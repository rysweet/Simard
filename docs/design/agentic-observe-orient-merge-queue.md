...
The default reasoning scope is Simard's governed-repos roster — the same
validated-slug, identity-scoped roster the
[`ecosystem-observe`](./ecosystem-observe.md) chain reads, including Simard's own
repo (`rysweet/Simard`). It is resolved through the single loader
`load_governed_roster` from durable, install-safe identity state (see
[Governed-roster resolution](../reference/ecosystem-roster-resolution.md)); an
empty roster is a **loud error**, never a silent empty scope.
...
/// the governed roster from Simard's identity-scoped state (load_governed_roster).
...
The reasoning scope roster is Simard's identity-scoped, agentically-curated
governed roster (data, not env), persisted durably at
`<state_root>/identity_state/simard/governed_repos.toml` and resolved via
`load_governed_roster`.
...
```bash
# Point the recipe at Simard's live governed roster + her in-flight refs, plus a
