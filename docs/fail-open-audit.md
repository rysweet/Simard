...
|-------|--------------|-------|
| PROPAGATE | 0 confirmed | None found; gym/knowledge clients are LOG-AND-CONTINUE by design |
| LOG-AND-CONTINUE | ~5 known | rpc_subprocess_launcher.rs (FIXED in this PR), cost_tracking.rs:280, dashboard goal_board parse, etc. |
| DOCUMENT-FAIL-OPEN | ~93 | meeting_repl writeln/flush (52), env-var lookups, file existence checks |

## Highest-leverage sites

### `src/rpc_subprocess_launcher.rs` — FIXED (originally in fail-open audit PR, updated in #2181)
...
- 1 PROPAGATE→LOG conversion shipped (rpc_subprocess_launcher)
