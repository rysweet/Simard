...
flowchart TD
    ...
    Mem --> OBS[OBSERVE<br/><i>context from goals +<br/>memory + adapters</i>]
...
|------|------|-------------|
| Daemon bootstrap | `src/operator_commands_ooda/daemon/mod.rs` | Wires adapters, brains, recipes; runs cycle loop |
| OODA cycle body | `src/ooda_loop/cycle.rs` | Observe → orient → decide → act → reflect |
| Brains | `src/ooda_reasoners/{rustyclawd,decide,orient,fallback}.rs` | LLM-backed + deterministic fallback |
| Prompt store | `src/ooda_reasoners/prompt_store.rs` | Disk load (mtime-cached) with embedded fallback |
