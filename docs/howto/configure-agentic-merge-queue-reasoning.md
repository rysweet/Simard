...
- Simard's governed roster (her identity-curated `governed_repos` dataset,
  durable at `~/.simard/state/identity_state/simard/governed_repos.toml`) is
  populated — it seeds from her identity on first use.
...
> `roster_path` is a context file of `owner/name` slugs; export Simard's durable
> `governed_repos` roster to it. On the live cadence the rail writes it for you.

## Troubleshooting

| Symptom | Cause | Fix |
|---|---|---|
| `reasoned_prs=0` with open PRs on the roster | roster empty or `gh` not authed | curate Simard's `governed_repos` roster; `gh auth status` |
