...
- [Distillation parse-failure-rate hardening](./reference/distill-parse-failure-rate-hardening.md) - The consolidated parse-resilience contract that drove the ~85% distill parse-failure rate back toward zero: the banner-immune facts-file channel, field-tolerant deserialization, the single-trailing-comma repair, strict rejection of genuinely malformed output, the first-class `distill_parse_success_rate` metric, and the `zero_facts_reason` disposition (`none` / `true_empty` / `all_quarantined`) that separates "nothing worth distilling" from "the reliability gate blocked everything".
