...
- [Distill trailing-comma recovery](./reference/distill-trailing-comma-recovery.md) - The string-aware `recipe_output::strip_json_trailing_commas` repair and the strict-first/repair-on-Err-only recovery tier in `scan_cleaned_for_facts` that rescue an otherwise valid facts document whose only defect is a JSON-illegal trailing comma — closing the `distill parse-fail rate 100%` signature — plus the `ParseRecovery` discriminator, its append-only `parse_recovery` key in `metrics.jsonl`, the distinct zero-facts-after-filter warn, and the never-a-hollow-Ok guarantee (issue #2669). See the [observe-and-verify how-to](./howto/observe-distill-trailing-comma-recovery.md).
