...
- [Distill trailing-comma parse recovery](./reference/distill-trailing-comma-recovery.md) - The string-aware, delete-only `strip_json_trailing_commas` recovery that repairs the distill agent's otherwise-valid JSON when it carries a trailing comma before a `}`/`]` — recover-only-on-`Cow::Owned` wiring into `scan_cleaned_for_facts`, the distinct zero-facts yield-loss warn that keeps parse-fail and yield-loss telemetrically separate, and the `distill_parse_success_rate` it drives back to `1.0` — clearing the recurring `anomaly:distill parse-fail rate 100%` signature (#2658 / #2672). See the [recover-from-trailing-comma runbook](./howto/recover-from-distill-trailing-comma-parse-failures.md).
