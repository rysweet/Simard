...
- [Goal-decomposition result channel](./reference/goal-decomposition-result-channel.md) — the decomposition agent hands its 2–6 sub-goals back through a dedicated, git-ignored result file (the `sub_goals_output` context var) instead of scraped `recipe-runner-rs` stdout, so runner noise can no longer discard a successful decomposition and a missing/malformed result fails loudly (#2708).
