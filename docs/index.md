...
- [Automatic distillation scheduler API](./reference/automatic-distillation-scheduler.md) - `maybe_run_promotion` / `should_distill` trigger predicate, the `SIMARD_DISTILL_MIN_EPISODES` / `SIMARD_DISTILL_INTERVAL_CYCLES` config, and the `DistilledProcedure` / `run_full` distillation procedures extension (issue #2327).
