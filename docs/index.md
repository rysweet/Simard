...
- [Distill zero-facts observability & parse-failure-rate recovery](./reference/distill-zero-facts-observability.md) - The content-free zero-facts log emitted from `RecipeEnvelope::into_facts` that makes a valid-but-empty distill result visible and distinguishes off-spec-concept contract drift from a legitimately empty envelope (without counting as a parse failure), plus the deterministic rate-verification harness that pins the residual 74%-era parse-failure rate to 0 across all three historical shapes (issue #2689).
