...
- [Concept: zero-fallback reasoners](./concepts/zero-fallback-reasoners.md) - The retcon narrative for the #2432 deterministic-fallback incident: why the Brain-Failures tab filled with deterministic-default outcomes (decide → `continue_skipping`, distill → 0 facts) and why the dashboard read zero active engineers, and how one sanitizing chokepoint, a required JSON-envelope decision contract per reasoner, explicit-error-plus-bounded-retry (never a silent default), a distinct evidenced take-no-action outcome, and a live-count engineer gauge honour the operator's absolute "no fallback" stance. Confirms the zero-engineers root cause is **REAL** (Decide never spawned) and downstream of the fallback bug, not a telemetry defect.
