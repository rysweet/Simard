...
- [Reference: trailing-comma-tolerant recipe-envelope JSON](./reference/distill-trailing-comma-tolerant-json.md) — the string-literal-aware `strip_json_trailing_commas` primitive and the fallback-only `parse_recipe_envelope` retry that recover a distill `{ "facts": [...] }` object emitted with a structural trailing comma, eliminating the recurring `anomaly:distill parse-fail rate 100%` signature that blocked the kgpacks-rs parity goals and skipped the quality gym gate (#2658, umbrella #2678). Strict `serde_json` still runs first; the retry is removal-only and fails closed. See [distill recipe-output capture](./reference/distill-recipe-output-capture.md) and [text-parsing wire formats](./reference/text-parsing-wire-formats.md).
