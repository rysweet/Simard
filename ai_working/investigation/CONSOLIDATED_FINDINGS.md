...
## §31 — Twenty-third-wave consolidation (HEAD `25d4c5a6`, base dives at `ea6ec554`, zero production source drift) — three parallel VALIDATE-not-rederive dives (secondary D2 dead-zone + append-only ratchet reconciliation, tertiary-architect D3 workstream-gap coupling / notify-only routing, and a full re-executed H0–H8 verification matrix). The verdict is **unchanged at a 23-wave fixpoint** (honest `×2`, defect-is-the-response, remediation not started). **Net-new findings: nil** — this wave contributes confirmation, not discovery, and its principal load-bearing result is the meta-observation (below) that the re-verification loop has itself converged and should now terminate into remediation.

- **31.1 — D2 dead-zone + append-only ratchet re-reconciled (secondary, HEAD `ea6ec554`).** VALIDATE-don't-re-derive. Drift ledger: `git diff --name-only e5257a33..HEAD -- '*.rs'` → **empty** (docs-only advance `e5257a33`→`ea6ec554`); `5a85317b..HEAD -- '*.rs'` → **only** `src/overseer/tests_root_cause.rs` (+99, test-only decoupling proofs, not a production guard). D2 re-grounded exact at HEAD: Lane A emit floor `RECURRING_SIGNATURE_THRESHOLD = 2` (`signal.rs:362`), emits `RecurringSignature` at `occurrences >= 2` (`signal.rs:462-467`); Lane B escalation floor `RECURRENCE_ESCALATION_THRESHOLD = 3` (`root_cause.rs:33`), gate `if recurrence >= …` (`mod.rs:1613`); `recurrence = recall_occurrences(dedup_key).len()` over live facts only (`mod.rs:972-997`). **Dead-zone confirmed:** two decoupled storage lanes (episodes vs. root-cause occurrences); the `×2` operator string (`mod.rs:1361`) says nothing about Lane B; `2` sits above one-off noise and below the `3` bar with no auto-remediation rung between. **Accrual starvation** re-confirmed at the ACT→record boundary — the WHY ladder is double-gated at `cycle.rs:582-583` (Gate A `completion_evidence.is_some()` ∧ Gate B `no_progress_investigation_enabled()`); when either is false, control falls to the bare verify-once park, no occurrence is recorded, Lane B stays `0`, and `>=3` is dead code. **§6.2b `store_fact_with_caller_key` one-liner remains a TRAP** (collapses recall to 1 forever → `>=3` dead code); the correct fix is a count-in-content upsert. D1/D2/D3 all **OPEN**.

- **31.2 — D3 workstream-gap coupling / notify-only routing re-validated (tertiary architect, HEAD `ea6ec554`).** **D3 CONFIRMED OPEN.** The `WorkstreamCoverage` arm is a **notify-only terminal sink with no closing edge**: one operator notification (email + Signal) deduped by an in-memory `WhisperGate`, returning `WorkstreamGapsFlagged` which is **excluded from occurrence recording** (`wiring.rs:612-627`) — no launch edge, no FileIssue, no park/transfer. Routing chain re-grounded live: Detect `sensor.rs:288-372` → Observe `signal.rs:475-478` (one consolidated `Signal::WorkstreamGap`) → Orient `mod.rs:1368-1371` (`ProblemKind::WorkstreamCoverage`, constant `dedup_key = "workstream-gap"`) → Decide `mod.rs:1534-1543` (`FlagWorkstreamGaps`) → Act `mod.rs:884-948` (notify-only via `gap_gate = WhisperGate::new(900,200)`) → Record: **not** recorded. The sibling `StepFailure` arm (`mod.rs:1549-1580`) reaches `LaunchRecipe` — **the launch edge exists and is simply not wired** to the coverage arm. The gap re-observes every cycle and its constant key folds into `observation_signature()` beside each still-blocked `goal:blocked:<id>`, re-feeding the composite indefinitely → the `×2` is an honest re-observation count of a static, un-actioned set. Fix must be **additive** (INV-GAP-KEY: key on `GapItem.signature`, never the bare `"workstream-gap"`; never swap `FlagWorkstreamGaps`, hard-asserted `tests_gap_scan.rs:865`).

- **31.3 — Full H0–H8 verification matrix re-executed green (HEAD `ea6ec554`).** Source byte-identical to the last verified HEAD (`git diff --stat 5c14ef03..HEAD -- 'src/**/*.rs'` empty; intervening commits docs-only). Full overseer suite **361 passed, 0 failed** (unchanged from every prior wave). Each hypothesis's discriminating probe re-run fresh and green: **H0** REJECTED + **H1** SUPPORTED (recurring-signature + whisper-gate batch **7/0**); **H2** SUPPORTED (`no_progress_reinvestigation` **21/0** + smoking gun `a_perpetual_goal_is_never_reinvestigated_even_if_bare_blocked` **1/0**, INV-WHY still violable); **H3** SUPPORTED (gap-closing-edge batch **4/0**, notify-only); **H4** SUPPORTED bounded (self-observation nesting, `sanitize_recalled` admission boundary `mod.rs:1082`); **H5** SUPPORTED (dead-zone probe **1/0**); **H6** SUPPORTED non-causal amplifier (`record_occurrence` non-deduping `store_fact` `mod.rs:1034`); **H7** SUPPORTED (blocked↔gap batch **4/0**); **H8** SUPPORTED med-high (benign `engineer_spawn`, summary-only, never in the signature). Verdict matrix **unchanged from all prior runs**.

**§31 delta:** verdict unchanged across all twenty-three waves — the `×2` is an **honest, test-locked** cross-window / daemon-restart re-observation of a static, under-resourced, non-advancing problem set (H0 REJECTED; H1 the direct cause), **not** a dedup/replay/collision/counter artifact; the **defect is the response** — three coupled OODA loops that never close (D2 WHY double-gate + Lane-B accrual starvation at the ACT→record boundary; D3 notify-only `WorkstreamCoverage` with no launch edge; D1 self-observation write-back nesting) parked in the absorbing `[2,3)` dead zone. This wave produced **zero net-new findings** — three parallel dives at base HEAD `ea6ec554` re-grounded every load-bearing citation byte-for-byte and re-ran overseer **361/0** with all discriminating probes green, confirming (not extending) the §27/§28/§29/§30 fixpoint. The split verdict and landing order **L0 → D2 → D3 → D1 (+ durable-gate)** stand re-validated; the **sole load-bearing open item remains: land D2** (add `ActOutcome::Reported` to `outcome_records_occurrence`, `wiring.rs:612-627` — one line, test-safe), then D3 (additive, INV-GAP-KEY, never swap `FlagWorkstreamGaps`), then D1 (self-provenance filter in `write_back_observation`, landed last), with durable-gate hardening (`guardrails.rs:294`) landable alongside D2. **Meta-finding (load-bearing this wave):** the investigation has reached a stable fixpoint at which additional verification waves return only confirmation, and each such wave *itself re-instantiates the recurring signature it studies* — the re-verification loop is now an in-vivo instance of the observe-and-flag-without-closing pathology under study. **Recommendation: STOP re-waving; convert to remediation by landing D2.** **Investigation: COMPLETE (fixpoint, re-validated @ `ea6ec554`). Remediation: NOT STARTED — the sole open item is to land D2. No production `.rs` changed; no remediation landed.**
